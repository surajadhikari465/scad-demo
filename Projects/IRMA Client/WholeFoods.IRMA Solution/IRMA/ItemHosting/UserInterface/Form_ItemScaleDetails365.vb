...
<Microsoft.VisualBasic.ComClass()> Public Class Form_ItemScaleDetails365
    ...
    Private Sub BindDataToControls()
        ...
        CheckBoxCfs.Checked = If(Me.ScaleDetailsBO.CustomerFacingScaleDepartment.HasValue, Me.ScaleDetailsBO.CustomerFacingScaleDepartment, False)
        CheckBoxSendToScale.Checked = If(Me.ScaleDetailsBO.SendToScale.HasValue, Me.ScaleDetailsBO.SendToScale, False)
    ...
    Private Function ApplyChanges() As Boolean
        ...
        Me.ScaleDetailsBO.CustomerFacingScaleDepartment = CheckBoxCfs.Checked

        Me.ScaleDetailsBO.IsNewToScale = IsNewToScale()
        Me.ScaleDetailsBO.SendToScale = CheckBoxSendToScale.Checked
        ...
        If Me.FormOwnsData Then
            ...
            Dim actionCode As String = If(ScaleDetailsBO.IsNewToScale, "A", "C")

            If ScaleDetailsBO.SendToScale Then
                ScaleDetailsDAO.GenerateCustomerFacingScaleMaintenance(ScaleDetailsBO.ItemKey, actionCode)
            End If
        End If
    ...
    Private Function IsNewToScale() As Boolean
        Return If(((Not ScaleDetailsBO.SendToScale.HasValue OrElse Not ScaleDetailsBO.SendToScale.Value) And CheckBoxSendToScale.Checked), True, False)
    End Function
    ...
    Private Sub Form_ItemScaleDetails_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        ...
        Dim IsEditable As Boolean = (gbItemAdministrator And frmItem.pbUserSubTeam) Or gbSuperUser
    ...
    Private Sub ButtonSaveScaleInformation_Click(sender As System.Object, e As System.EventArgs) Handles ButtonSaveScaleInformation.Click
        ...
        Select Case SelectedTab
            Case "General"
                ...
                End If
            Case "Nutrifacts"
                If Me.ApplyNutrifactsChanges() And ApplyChanges() Then
    ...
    Private Sub LoadNutrifacts(ByVal NutrifactID As Integer)
        ...
        'RemoveHandler NutrifactCombo.SelectedIndexChanged, AddressOf DescriptionCombo_SelectedIndexChanged
        NutrifactCombo.DataSource = ScaleNutrifactDAO.GetNutrifactComboList()
        'AddHandler NutrifactCombo.SelectedIndexChanged, AddressOf DescriptionCombo_SelectedIndexChanged

        If NutrifactCombo.Items.Count > 0 Then
            'Dim scaleNutrifact As New ScaleNutrifactBO()
            'scaleNutrifact = CType(NutrifactCombo.SelectedItem, ScaleNutrifactBO)
            ...
            If NutrifactID > 0 Then
                ...
                'PopulateNutrifactsControls(CInt(NutrifactCombo.SelectedValue.ToString()))
...
#Region "Nutrifacts Controls"
    Private Sub NutrifactCombo_SelectionChangeCommitted(sender As Object, e As EventArgs) Handles NutrifactCombo.SelectionChangeCommitted
        SetNutrifactsDataChanges(True)
    End Sub
    ...
    Private Sub ButrifactDescriptionTextbox_TextChanged(sender As Object, e As EventArgs) Handles NutrifactDescriptionTextbox.TextChanged
        SetNutrifactsDataChanges(True)
    End Sub
...
#End Region
    ...
    Private Function ApplyNutrifactsChanges() As Boolean
        ...
        If hasNutrifactsChanges = True Then
            ...
            If ScaleNutrifactDAO.Save(scaleNutrifact, alternateJurisdiction) Then
                ...
                If ScaleDetailsBO.Nutrifact = -1 Then
                    ScaleDetailsBO.Nutrifact = Me.NutrifactsID
                End If
    ...
    Private Sub CheckBoxCfs_CheckStateChanged(sender As Object, e As EventArgs) Handles CheckBoxCfs.CheckStateChanged
        SetDataChanges(True)
    End Sub

    Private Sub CheckBoxSendToScale_CheckStateChanged(sender As Object, e As EventArgs) Handles CheckBoxSendToScale.CheckStateChanged
        SetDataChanges(True)
    End Sub

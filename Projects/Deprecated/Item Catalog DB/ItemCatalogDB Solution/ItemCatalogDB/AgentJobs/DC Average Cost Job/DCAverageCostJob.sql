...
IF @Environment = 'TEST'
	BEGIN
		BEGIN TRANSACTION
		SELECT @ReturnCode = 0
		/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 07/06/2009 15:07:19 ******/
		...
		EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DC Average Cost Import', 
				...
				--@notify_email_operator_name=N'DC Average Cost Import', 
				@job_id = @jobId OUTPUT
		IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
		/****** Object:  Step [Update DC Average Cost]    Script Date: 07/06/2009 15:07:20 ******/

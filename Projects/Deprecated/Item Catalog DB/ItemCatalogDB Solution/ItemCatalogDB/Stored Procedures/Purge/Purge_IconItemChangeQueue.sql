IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Purge_IconItemChangeQueue'))
   exec('CREATE PROCEDURE [dbo].[Purge_IconItemChangeQueue] AS BEGIN SET NOCOUNT ON; END')
GO

ALTER PROCEDURE [dbo].[Purge_IconItemChangeQueue] (@QC BIT = 1, @BatchSize INT) AS BEGIN
	...
	SELECT @Rowset = COUNT(*) FROM [dbo].[IconItemChangeQueue] WHERE InsertDate < @Then
	...
	WHILE @UpdateCt_Total < @TotalCt BEGIN
		...
		BEGIN TRAN
			BEGIN TRY
				IF (@QC = 1) BEGIN
					SELECT TOP (@BatchSize) * FROM [dbo].[IconItemChangeQueue] WHERE InsertDate < @Then
					SELECT @UpdateCt_Batch = @@ROWCOUNT	
				END

				IF (@QC = 0) BEGIN
					DELETE TOP (@BatchSize) FROM [dbo].[IconItemChangeQueue] WHERE InsertDate < @Then
					SELECT @UpdateCt_Batch = @@ROWCOUNT	
				END

IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Purge_MammothChangeQueueHistory'))
   exec('CREATE PROCEDURE [dbo].[Purge_MammothChangeQueueHistory] AS BEGIN SET NOCOUNT ON; END')
GO

ALTER PROCEDURE [dbo].[Purge_MammothChangeQueueHistory] (@QC BIT = 1, @BatchSize INT) AS BEGIN
	...
	SELECT @Rowset = COUNT(*) FROM [mammoth].[ChangeQueueHistory] WHERE InsertDate < @Then
	...
	WHILE @UpdateCt_Total < @TotalCt BEGIN
		...
		BEGIN TRAN
			BEGIN TRY
				IF (@QC = 1) BEGIN
					SELECT TOP (@BatchSize) * FROM [mammoth].[ChangeQueueHistory] WHERE InsertDate < @Then
					SELECT @UpdateCt_Batch = @@ROWCOUNT	
				END

				IF (@QC = 0) BEGIN
					DELETE TOP (@BatchSize) FROM [mammoth].[ChangeQueueHistory] WHERE InsertDate < @Then
					SELECT @UpdateCt_Batch = @@ROWCOUNT	
				END

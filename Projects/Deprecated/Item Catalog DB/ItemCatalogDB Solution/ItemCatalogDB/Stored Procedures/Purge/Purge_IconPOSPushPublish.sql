IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.Purge_IconPOSPushPublish'))
   exec('CREATE PROCEDURE [dbo].[Purge_IconPOSPushPublish] AS BEGIN SET NOCOUNT ON; END')
GO

ALTER PROCEDURE [dbo].[Purge_IconPOSPushPublish] (@QC BIT = 1, @BatchSize INT) AS BEGIN
	...
	SELECT @Rowset = COUNT(*) FROM [dbo].[IConPOSPushPublish] WHERE InsertDate < @Then
	...
	WHILE @UpdateCt_Total < @TotalCt BEGIN
		...
		BEGIN TRAN
			BEGIN TRY
				IF (@QC = 1) BEGIN
					SELECT TOP (@BatchSize) * FROM [dbo].[IConPOSPushPublish] WHERE InsertDate < @Then
					SELECT @UpdateCt_Batch = @@ROWCOUNT	
				END

				IF (@QC = 0) BEGIN
					DELETE TOP (@BatchSize) FROM [dbo].[IConPOSPushPublish] WHERE InsertDate < @Then
					SELECT @UpdateCt_Batch = @@ROWCOUNT	
				END

	...
	if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[GetPurchaseOrderHeader]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
	drop procedure [dbo].[GetPurchaseOrderHeader]
	GO
	...
	END
	GO
	--GRANT EXEC ON dbo.GetPurchaseOrderHeader TO IRMAAdminRole, IRMAClientRole, IRMAReportsRole, IRMASchedJobsRole, IRMASupportRole
	--GO

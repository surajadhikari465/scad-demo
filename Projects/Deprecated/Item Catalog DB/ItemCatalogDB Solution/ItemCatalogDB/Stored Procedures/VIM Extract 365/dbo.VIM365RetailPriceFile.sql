IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID(N'dbo.VIM365RetailPriceFile'))
	EXEC('CREATE PROCEDURE [dbo].[VIM365RetailPriceFile] AS BEGIN SET NOCOUNT ON; END')
GO

ALTER PROCEDURE [dbo].[VIM365RetailPriceFile]
AS
BEGIN
--**************************************************************************
-- Procedure: VIM365RetailPriceFile()
--    Author: N/A
--      Date: N/A
--
-- Description:
...
IF object_id('dbo.VIM365RetailPriceFileLoad') IS NOT NULL 
DROP TABLE dbo.VIM365RetailPriceFileLoad

IF EXISTS (SELECT name FROM dbo.sysindexes WHERE name = N'[dbo].[idxVIM365RetailPriceFileLoad]')
DROP INDEX [dbo].idxVIM365RetailPriceFileLoad

CREATE TABLE dbo.VIM365RetailPriceFileLoad (
...
INSERT INTO dbo.VIM365RetailPriceFileLoad
	SELECT
	...
	AND (@ExcludeSKUIdentifiers = 0 OR (@ExcludeSKUIdentifiers = 1 AND Identifier.IdentifierType <> 'S'))

CREATE CLUSTERED INDEX idxVIM365RetailPriceFileLoad ON dbo.VIM365RetailPriceFileLoad
(
	[UPC] ASC,
	[REGION] ASC,
	[POS_DEPT] ASC,
...
INSERT INTO @VIMSamePrice
	SELECT UPC FROM
		(
			SELECT DISTINCT UPC, REG_PRICE, EFF_PRICE, REG_MULTIPLE, EFF_MULTIPLE, START_DATE, END_DATE FROM dbo.VIM365RetailPriceFileLoad
		) AS T1
	GROUP BY UPC
	HAVING COUNT (*) = 1
...
INNER JOIN dbo.VIM365RetailPriceFileLoad VRPL ON VRPL.UPC = SP.UPC

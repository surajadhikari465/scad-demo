...
IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'dbo.GetPriceBatchItemSearch') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[GetPriceBatchItemSearch]
GO
			...
			ELSE 
				IF @Status = 3 -- Both Validated UPC & PLU are passing from Icon to IRMA
					BEGIN				
						...
						FROM Item I (NOLOCK) INNER JOIN ItemIdentifier II (NOLOCK)
						ON I.Item_Key = II.Item_Key
						WHERE (I.Remove_Item = 0 AND I.Delete_Item = 0 AND II.Remove_Identifier = 0) AND I.Retail_Sale = 0
					END	

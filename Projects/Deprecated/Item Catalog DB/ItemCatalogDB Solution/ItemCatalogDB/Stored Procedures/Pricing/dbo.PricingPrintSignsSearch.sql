if not exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[PricingPrintSignsSearch]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
	EXEC('CREATE PROCEDURE [dbo].[PricingPrintSignsSearch] AS BEGIN SET NOCOUNT ON; END')
GO

ALTER PROCEDURE [dbo].[PricingPrintSignsSearch]
    @StoreNumber int,
    @SubteamNumber int,
    @CategoryId int,

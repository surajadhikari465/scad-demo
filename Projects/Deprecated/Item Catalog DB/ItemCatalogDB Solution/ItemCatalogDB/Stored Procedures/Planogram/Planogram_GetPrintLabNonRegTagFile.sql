/****** Object:  StoredProcedure [dbo].[Planogram_GetPrintLabNonRegTagFile]    Script Date: 06/22/2007 11:08:20 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Planogram_GetPrintLabNonRegTagFile]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Planogram_GetPrintLabNonRegTagFile]
GO

CREATE PROCEDURE [dbo].[Planogram_GetPrintLabNonRegTagFile]
...
DEALLOCATE pbd_cursor
	...
	BEGIN CATCH
		...
		RAISERROR ('Planogram_GetPrintLabNonRegTagFile failed with  @ErrorMessage' , -- Message text.

...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Planogram_GetFXSetRegTagFile]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Planogram_GetFXSetRegTagFile]
GO

CREATE PROCEDURE [dbo].[Planogram_GetFXSetRegTagFile]
...
BEGIN
    ...
    SELECT DISTINCT
		...
		CAST( CAST (P.MSRPMultiple AS varchar(3)) + '/'+ CAST(P.MSRPPrice As Varchar(8)) As VARCHAR(12)) As Price,
...
UNION
    SELECT DISTINCT
      ...
      FROM 
		dbo.Planogram PL (nolock)
		JOIN dbo.Store ST (nolock) ON PL.Store_No = ST.PSI_Store_No 
		...
		JOIN dbo.Price P (nolock) ON P.Item_Key = PL.Item_Key AND P.Store_No = ST.Store_No
		JOIN dbo.StoreItem SI (nolock) ON ST.Store_no=SI.Store_No AND I.Item_Key=SI.Item_Key 
        ...
        WHERE 
			ST.Store_No = @Store_No  AND SI.Authorized = 1
			AND P.PriceChgTypeID <> @PriceChgTypeID
 			AND SIV.PrimaryVendor = 1
			AND VCH.VendorCostHistoryID = (Select Top 1 VCH2.VendorCostHistoryID 
					FROM dbo.VendorCostHistory VCH2 (nolock) 
					...
					ORDER BY VCH2.VendorCostHistoryID Desc) 
		ORDER BY 
			-- 2.11.10 - What's the point of trying to print planogram tags in an order other than planogram set order?
			--LabelTypeDesc,
			--ExemptTagDesc,
			PL.ProductPlanogramCode, 
			PL.ProductGroup, 
	...
	BEGIN CATCH
		...
		RAISERROR ('Planogram_GetFXSetRegTagFile failed with  @ErrorMessage' , -- Message text.

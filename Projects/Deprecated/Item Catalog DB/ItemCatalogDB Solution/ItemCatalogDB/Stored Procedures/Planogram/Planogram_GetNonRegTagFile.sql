IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Planogram_GetNonRegTagFile]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Planogram_GetNonRegTagFile]
GO

CREATE PROCEDURE [dbo].[Planogram_GetNonRegTagFile]
...
IF @POSFileWriterClass like '%PrintLab%' --PrintLab
	BEGIN
		...
		EXEC dbo.Planogram_GetPrintLabNonRegTagFile @ItemList, @ItemListSeparator, @Store_No, @StartDate
	END
ELSE IF  @POSFileWriterClass like '%FileMaker%' --FileMaker
	BEGIN
		--PRINT 'FileMaker ' + @POSFileWriterCode
		EXEC dbo.Planogram_GetFileMakerNonRegTagFile @ItemList, @ItemListSeparator, @Store_No, @StartDate
	END
ELSE IF  @POSFileWriterClass like '%FX%' --FileMaker
	BEGIN
		--PRINT 'FileMaker ' + @POSFileWriterCode
		EXEC dbo.Planogram_GetFXNonRegTagFile @ItemList, @ItemListSeparator, @Store_No, @StartDate
	END
ELSE IF @POSFileWriterClass = 'TagWriter' AND @POSFileWriterCode LIKE '%AccessVia%' -- AccessVia
	BEGIN
		...
		IF @POSFileWriterCode LIKE '%AccessViaExt%' -- AccessViaExt
				BEGIN
					EXEC dbo.Planogram_GetAccessViaExtendedNonRegTagFile @ItemList, @ItemListSeparator, @Store_No, @StartDate
				END
			ELSE
				BEGIN
		            EXEC dbo.Planogram_GetAccessViaNonRegTagFile @ItemList, @ItemListSeparator, @Store_No, @StartDate
				END
	END
	...
	BEGIN CATCH
		...
		RAISERROR ('Planogram_GetNonRegTagFile failed with  @ErrorMessage' , -- Message text.

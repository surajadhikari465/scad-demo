IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Planogram_GetFileMakerNonRegTagFile]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Planogram_GetFileMakerNonRegTagFile]
GO

CREATE PROCEDURE [dbo].[Planogram_GetFileMakerNonRegTagFile]
...
DEALLOCATE pbd_cursor
    ...
    SELECT DISTINCT
      ...
      FROM 
		...
		JOIN dbo.Planogram PL (nolock) ON PBD.Item_Key = PL.Item_Key AND  PL.Store_No = @Store_No
		...
		CROSS APPLY (Select ShelfTag_Type, Exempt_ShelfTag_Type, ShelfTag_Ext, ShelfTag_Ext2 from dbo.fn_getShelfTagType (PBD.Item_Key,I.LabelType_ID,ISNULL(PBD.PriceChgTypeID, P.PriceChgTypeID),PBD.ItemChgTypeID,I.SubTeam_No,st.TaxJurisdictionID,st.Store_No, St.Zone_ID, @UseExempt, @UseEST)) AS TAG
	...
	BEGIN CATCH
		...
		RAISERROR ('Planogram_GetFileMakerNonRegTagFile failed with  @ErrorMessage' , -- Message text.

IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'InsertItemLocaleChangeQueueByBatchHeaderAndStore')
	EXEC('CREATE PROCEDURE [mammoth].[InsertItemLocaleChangeQueueByBatchHeaderAndStore] AS BEGIN SET NOCOUNT ON; END')
GO

ALTER PROCEDURE [mammoth].[InsertItemLocaleChangeQueueByBatchHeaderAndStore]

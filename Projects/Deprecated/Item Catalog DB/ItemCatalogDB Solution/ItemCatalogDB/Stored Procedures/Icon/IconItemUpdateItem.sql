...
--				updates the Item table with consolidated data
-- =============================================

IF NOT EXISTS (SELECT * FROM sys.objects WHERE OBJECT_ID = OBJECT_ID('IconItemUpdateItem') AND type in (N'P', N'PC'))
BEGIN
	EXEC ('CREATE PROCEDURE [dbo].[IconItemUpdateItem] as SELECT 1')
END
GO

ALTER PROCEDURE [dbo].[IconItemUpdateItem]

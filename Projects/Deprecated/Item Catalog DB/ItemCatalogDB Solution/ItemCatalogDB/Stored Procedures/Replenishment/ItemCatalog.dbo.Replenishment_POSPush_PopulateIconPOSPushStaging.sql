IF NOT EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[Replenishment_POSPush_PopulateIconPOSPushStaging]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
	EXEC ('create PROCEDURE [dbo].[Replenishment_POSPush_PopulateIconPOSPushStaging] (@foo int) as select 1')
GO

ALTER PROCEDURE dbo.Replenishment_POSPush_PopulateIconPOSPushStaging
	(

IF NOT EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[Replenishment_POSPush_PopulateBatchIconPOSPushStaging]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
	EXEC ('create PROCEDURE [dbo].[Replenishment_POSPush_PopulateBatchIconPOSPushStaging] (@foo int) as select 1')
go 

ALTER  PROCEDURE [dbo].[Replenishment_POSPush_PopulateBatchIconPOSPushStaging]
	@Date DATETIME,
	@MaxBatchItems INT,
	@Deletes BIT,
	@IsScaleZoneData BIT

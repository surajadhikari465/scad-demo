IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Replenishment_TagPush_GetFullElectronicShelfTagFile]') AND type in (N'P', N'PC'))
begin
	EXEC ('CREATE PROCEDURE [dbo].[Replenishment_TagPush_GetFullElectronicShelfTagFile] AS SELECT 1')
end
GO
...
ALTER PROCEDURE [dbo].[Replenishment_TagPush_GetFullElectronicShelfTagFile]
	@StoreNo int
AS
...
BEGIN
	...
	SELECT 
		...
		@IncludeNonDefaultIdentifiers = dbo.fn_InstanceDataValue('IncludeAllItemIdentifiersInShelfTagPush', @StoreNo)
	...
	SELECT
		...
		[Dept_No] = ST.[Dept_No],
		[RetailUnit] = COALESCE(iuoviu.Unit_Abbreviation, IUR.Unit_Abbreviation),
	...
	FROM 
		...
		LEFT JOIN dbo.ItemUomOverride	iuov				(NOLOCK) ON I.Item_Key				= iuov.Item_Key 
																		AND S.Store_No			= iuov.Store_No
		LEFT JOIN dbo.ItemUnit			iuoviu				(NOLOCK) ON iuov.Retail_Unit_ID		= iuoviu.Unit_ID
		LEFT JOIN dbo.ItemSignAttribute ISA					(NOLOCK) ON I.Item_Key				= ISA.Item_Key 
		LEFT JOIN dbo.ItemNutrition		ITN					(NOLOCK) ON I.Item_Key				= ITN.ItemKey		
		LEFT JOIN dbo.Nutrifacts		NonScaleNutrifacts	(NOLOCK) ON ITN.NutrifactsID		= NonScaleNutrifacts.NutriFactsID 
		LEFT JOIN dbo.Scale_Ingredient	NonScaleIngredients (NOLOCK) ON ITN.Scale_Ingredient_ID = NonScaleIngredients.Scale_Ingredient_ID
		LEFT JOIN dbo.Scale_Allergen	NonScaleAllergen	(NOLOCK) ON ITN.Scale_Allergen_ID	= NonScaleAllergen.Scale_Allergen_ID
		LEFT JOIN dbo.ItemScale			ISC					(NOLOCK) ON I.Item_Key				= ISC.Item_Key
		LEFT JOIN dbo.Nutrifacts		ScaleNutrifacts		(NOLOCK) ON ISC.Nutrifact_ID		= ScaleNutrifacts.NutriFactsID
		LEFT JOIN dbo.Scale_Ingredient	ScaleIngredients	(NOLOCK) ON ISC.Scale_Ingredient_ID = ScaleIngredients.Scale_Ingredient_ID
		LEFT JOIN dbo.Scale_Allergen	ScaleAllergen		(NOLOCK) ON ISC.Scale_Allergen_ID	= ScaleAllergen.Scale_Allergen_ID
	WHERE 

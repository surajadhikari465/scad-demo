IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'dbo.Replenishment_ScalePush_GetNutriFactChanges') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE dbo.Replenishment_ScalePush_GetNutriFactChanges
GO
...
BEGIN  
	...
	DECLARE @ExtraTextChangeType INT = (SELECT TOP 1 POSChangeTypeKey FROM dbo.POSChangeType WHERE ChangeTypeDesc = 'NutriFact Data')

IF NOT EXISTS (	SELECT * FROM dbo.sysobjects WHERE OBJECTPROPERTY(id, N'IsProcedure') = 1 AND
	id = OBJECT_ID(N'[dbo].[Replenishment_POSPush_PublishPriceBatchDenorm]') 
)
BEGIN
	EXEC ('CREATE PROCEDURE dbo.Replenishment_POSPush_PublishPriceBatchDenorm AS SELECT 1')
END
GO
...
ALTER PROCEDURE dbo.Replenishment_POSPush_PublishPriceBatchDenorm

IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'Scale_InsertUpdateExtraText')
	EXEC('CREATE PROCEDURE [dbo].[Scale_InsertUpdateExtraText] AS BEGIN SET NOCOUNT ON; END')
GO

ALTER PROCEDURE dbo.Scale_InsertUpdateExtraText
	@ID int,
		...
		BEGIN
			...
			IF @Error_No = 0
				BEGIN
					...
					IF EXISTS (SELECT * FROM @IdentifiersTable)
						EXEC mammoth.GenerateEvents @IdentifiersTable,'ItemLocaleAddOrUpdate', NULL
				END
			...
			IF @Error_No = 0
				BEGIN
					...
					IF EXISTS (SELECT * FROM @IdentifiersTable)
						EXEC mammoth.GenerateEvents @IdentifiersTable,'ItemLocaleAddOrUpdate', @StoreJurisdictionID
				END
		END
	ELSE
		BEGIN

/****** Object:  StoredProcedure [dbo].[Reporting_CostAudit]    ******/
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Reporting_CostAudit]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].[Reporting_CostAudit]

/****** Object:  StoredProcedure [dbo].[Reporting_CostAudit] ******/

set QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Reporting_CostAudit]
@Vendor_ID int,
@Zone_ID int,
@IsRegional bit,
...
AS
--**************************************************************************
-- Procedure: Reporting_CostAudit
--
-- Revision:
-- 01/10/2013  MZ    TFS 8755 - Replace Item.Discontinue_Item with   
--                   StoreItemVendor.DiscontinueItem, and add it to @AvailStoreItemVendor.
--**************************************************************************
BEGIN
SET NOCOUNT ON
...
----------------------------------------------
INSERT INTO #tblVendorCostItemsStores (Item_Key, Store_No, Vendor_ID, UnitCost, UnitFreight, Package_Desc1, StartDate, EndDate, InsertDate)
...
-- Create a temp table
----------------------------------------------
INSERT INTO @AvailStoreItemVendor (StoreItemVendorID, Store_No, Store_Name, Zone_Name, Item_Key, Vendor_ID, RegCost, NetCost, Discount, PriceExcVAT, PriceIncVAT, PromotionStatus, StartDate, EndDate, InsertDate, DiscontinueItem)
SELECT SIV.StoreItemVendorID, S.Store_No, S.Store_Name,
Z.Zone_Name,
SIV.Item_Key,
SIV.Vendor_ID,
--NET COST = REG COST - NET DISCOUNT + Freight
ISNULL(VC.UnitCost, 0) AS RegCost,
...
ISNULL(dbo.fn_GetCurrentSumAllowances(SIV.Item_Key, S.Store_No),0) As Discount,
...
ON VDH.StoreItemVendorID = SIV.StoreItemVendorID AND @CurrDate between VDH.StartDate and VDH.EndDate
...
I.Package_Desc1,
I.Package_Desc2,
CAST ((Select top 1  Package_Desc1 from VendorCostHistory where StoreItemVendorID = ASIV.StoreItemVendorID
			ORDER BY InsertDate DESC) AS INT) AS CaseSize,
IU.Unit_Name AS Package_Unit,
I.TaxClassID AS VAT_Flag,
--ASIV.Store_Name,
ASIV.RegCost AS Cost,
ASIV.Discount As Discount,
/*
ASIV.PriceIncVAT AS Price,
(CASE WHEN ISNULL(ASIV.PriceExcVAT, 0) <> 0
THEN (ASIV.PriceExcVAT - (ASIV.NetCost / I.Package_Desc1))/ASIV.PriceExcVAT  --DIVIDE NET COST BY PACK DESC 1 TO GET ITEM COST
END) AS Margin, 
...
ORDER BY V.CompanyName, IC.Category_Name, I.Item_Description
SET NOCOUNT OFF
END
GO
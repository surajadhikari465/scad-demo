IF NOT EXISTS (	SELECT * FROM dbo.sysobjects WHERE OBJECTPROPERTY(id, N'IsProcedure') = 1 AND
	id = OBJECT_ID(N'[dbo].[LoadInventoryServiceExport]') 
)
BEGIN
	EXEC ('CREATE PROCEDURE dbo.LoadInventoryServiceExport AS SELECT 1')
END
GO

ALTER  PROCEDURE [dbo].[LoadInventoryServiceExport]
AS 
BEGIN
    SET NOCOUNT ON

IF NOT EXISTS (	SELECT * FROM dbo.sysobjects WHERE OBJECTPROPERTY(id, N'IsProcedure') = 1 AND
	id = OBJECT_ID(N'[dbo].[LoadInventoryServiceImport]') 
)
BEGIN
	EXEC ('CREATE PROCEDURE dbo.LoadInventoryServiceImport AS SELECT 1')
END
GO

ALTER PROCEDURE dbo.LoadInventoryServiceImport

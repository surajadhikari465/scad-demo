IF NOT EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[PDX_FutureCostFile]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
	EXEC ('create PROCEDURE [dbo].[PDX_FutureCostFile] (@foo int) as select 1')
GO

ALTER PROCEDURE dbo.PDX_FutureCostFile
AS
BEGIN
	SET NOCOUNT ON

...
if not exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[GetOrderItemQueueSearch]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	EXEC ('CREATE PROCEDURE [dbo].[GetOrderItemQueueSearch] AS SELECT 1')
end
GO

ALTER PROCEDURE [dbo].[GetOrderItemQueueSearch]
	@OrderType_ID			int,
	@VendorID				int,

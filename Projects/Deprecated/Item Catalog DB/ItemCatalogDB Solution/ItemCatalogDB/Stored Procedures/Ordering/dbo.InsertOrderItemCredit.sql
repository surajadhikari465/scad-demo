...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[InsertOrderItemCredit]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[InsertOrderItemCredit]
GO
...
-- 02/10/2014   FA      2552    Added code to use hosted cost if there is @cost = 0
-- ****************************************************************************************************************
BEGIN
    SET NOCOUNT ON
...
--**************************************************************************    
   ...
   -- If @Cost is 0 then use hosted cost 
    IF @OrderType_ID = 3 AND ISNULL(@Cost, 0) = 0
		BEGIN 
			DECLARE	@VendStore_No	int			
			DECLARE @vendorpacksize	decimal(9,4)
			DECLARE @casepackcost	money
		
			SELECT	@VendStore_No = Store_No FROM	Vendor v (nolock) WHERE v.Vendor_ID = @Vendor_ID	 

IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[fn_GetCurrentNetUnitCost]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
DROP FUNCTION [dbo].[fn_GetCurrentNetUnitCost]
...
BEGIN
	...
	RETURN isnull(@CurrentNetUnitCost, 0)
END
GO

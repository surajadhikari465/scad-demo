...
-- Description :	ThreeWayMisMatchedDetailSummaryReport
-- ================================================================================= 

CREATE PROCEDURE [dbo].[ThreeWayMisMatchedDetailSummaryReport]
@ControlGroup_ID int,
@PoNumber_Id int,
@BeginDate DateTime,
@EndDate DateTime
		...
		BEGIN
		  -- Fetching the status directly from OrderHeader and OrderInvoice_Freight3Party Tables.
			  Select 
				...
				INNER JOIN dbo.ValidationCodeType VCT ON
				VC.ValidationCodeType=VCT.ValidationCodeType
				Where OH.OrderHeader_ID=ISNULL(@PoNumber_ID,OH.OrderHeader_ID)
				and OH.MatchingValidationCode in (select validationcode from validationcode where validationcodetype=2)
		   ...
		   union 
			   Select 
				...
				and MatchingValidationCode in (select validationcode from validationcode where validationcodetype=2)
	...
	ELSE
		BEGIN
			Select 
				...
				(OH.MatchingValidationCode=VC.ValidationCode and OH.MatchingValidationCode in (select validationcode from validationcode where validationcodetype=2))
				INNER JOIN dbo.ValidationCodeType VCT ON
				VC.ValidationCodeType=VCT.ValidationCodeType
		   ...
		   UNION 
			  Select 
				...
				INNER JOIN dbo.validationCode VC ON
				(TF.MatchingValidationCode=VC.ValidationCode and TF.MatchingValidationCode in (select validationcode from validationcode where validationcodetype=2))
				INNER JOIN dbo.ValidationCodeType VCT ON
				VC.ValidationCodeType=VCT.ValidationCodeType
				where CGI.OrderInvoice_ControlGroup_ID=
				ISNULL(@ControlGroup_ID,CGI.OrderInvoice_ControlGroup_ID)
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[ThreeWayMisMatchedDetailSummaryReport] TO [IRMAClientRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[ThreeWayMisMatchedDetailSummaryReport] TO [IRMASchedJobsRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[ThreeWayMisMatchedDetailSummaryReport] TO [IRMAReportsRole]
    AS [dbo];

CREATE Procedure [dbo].[UpdateOrderRefreshCostsSRT]
	@OrderHeader_ID		int,
...
-- **************************************************************************
-- NOTE: This is a special copy for Store Receive Tool to address INC1699896 (remove global scope for "IL" cursor).  This should only be called by updated Huckleberry > (special copy) UpdateOrderClosedSRT.
-- Changes are to the cursor's name and declared attributes.
--
...
BEGIN          
	...
	BEGIN TRY                       
		BEGIN TRANSACTION       
			...
			PRINT '## UpdateOrderRefreshCostsSRT: ' + cast(@OrderHeader_ID as varchar(100))
			...
			*/  
			...
			DECLARE	IL_SRT CURSOR LOCAL STATIC READ_ONLY FORWARD_ONLY FOR SELECT	
				OrderItem_ID,         
				oi.Item_Key,         
				Cost,         
				Freight,         
			...
			OPEN IL_SRT FETCH NEXT FROM IL_SRT INTO	
				@OrderItem_ID,         
				@Item_Key,         
				@Cost,         
				@Freight,         
	...
	BEGIN CATCH  
		...
		RAISERROR ('UpdateOrderRefreshCostsSRT failed with @@ERROR: %d - %s', @err_sev, 1, @err_no, @err_msg)  
	END CATCH     

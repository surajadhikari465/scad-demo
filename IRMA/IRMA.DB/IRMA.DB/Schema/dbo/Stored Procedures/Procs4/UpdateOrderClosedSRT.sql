CREATE PROCEDURE [dbo].[UpdateOrderClosedSRT] 
	@OrderHeader_ID		int,
	@User_ID			int,
...
-- ********************************************************************************************************
-- NOTE: This is a special copy for Store Receive Tool to address INC1699896 (remove global scope for "IL" cursor).
--
...
BEGIN
	...
	BEGIN TRAN
		...
		IF @Error_No = 0
			BEGIN
				EXEC UpdateOrderRefreshCostsSRT @OrderHeader_ID, 'UpdateOrderClosed', @User_ID, @IsSuspended OUTPUT
			END
	...
	ELSE
		BEGIN
			...
			SET NOCOUNT OFF
			RAISERROR ('UpdateOrderClosedSRT failed with @@ERROR: %d', @Severity, 1, @Error_No)
		END
END
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[UpdateOrderClosedSRT] TO [IRMAClientRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[UpdateOrderClosedSRT] TO [IRMASchedJobsRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[UpdateOrderClosedSRT] TO [IRMAReportsRole]
    AS [dbo];

GO
GRANT EXECUTE
    ON OBJECT::[dbo].[UpdateOrderClosedSRT] TO [PrimsReceivingRole]
    AS [dbo];
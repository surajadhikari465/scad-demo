CREATE PROCEDURE [dbo].[Reporting_VendorItems] 
	@Vendor_ID int,
	@Zone_ID int,
	@IsRegional bit,
...
AS 
--**************************************************************************
-- Procedure: Reporting_VendorItems
--
-- Revision:
...
BEGIN
	...
	DECLARE @AvailStoreItemVendor table(
									...
									PromotionStatus varchar(20) --Change by sekhara to fix 6417.(from varchar(7))
									)
	...
	-- Create a temp table
	----------------------------------------------
	INSERT INTO @AvailStoreItemVendor (Store_Name, Zone_Name, Item_Key, Vendor_ID, StoreItemVendorID, NetCost, PriceExcVAT, PriceIncVAT, PromotionStatus)
	SELECT S.Store_Name, 
		Z.Zone_Name, 
		SIV.Item_Key, 
		SIV.Vendor_ID,	
	...
	----------------------------------------------
	----------------------------------------------
	-- Tom.Lux@wholefoods.com
	-- 2.Nov.2007
	-- Bug 4292
	-- Change: Changed pack size being returned from i.package_desc1 to vch.package_desc1 so that report shows vendor case pack size.
	----------------------------------------------
	SELECT DISTINCT 
		ST.SubTeam_Name,
		IC.Category_Name,
	...
	WHERE I.Deleted_Item = 0 
		...
		AND
			(
			I.Category_ID = ISNULL(@Category_ID, I.Category_ID)
			OR
			I.Category_ID IS NULL
			) 
		AND
			(
			I.Brand_ID = ISNULL(@Brand_ID, I.Brand_ID)
			OR 
			I.Brand_ID IS NULL
			) 
        AND 
			(
			lv3.ProdHierarchyLevel3_ID = ISNULL(@Level3_ID, lv3.ProdHierarchyLevel3_ID)
			OR
			lv3.ProdHierarchyLevel3_ID IS NULL
			)
        AND 
			(
			lv4.ProdHierarchyLevel4_ID = ISNULL(@Level4_ID, lv4.ProdHierarchyLevel4_ID)
			OR 
			lv4.ProdHierarchyLevel4_ID IS NULL
			)
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[Reporting_VendorItems] TO [IRMAAdminRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[Reporting_VendorItems] TO [IRMASupportRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[Reporting_VendorItems] TO [IRMAClientRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[Reporting_VendorItems] TO [IRMAReportsRole]
    AS [dbo];

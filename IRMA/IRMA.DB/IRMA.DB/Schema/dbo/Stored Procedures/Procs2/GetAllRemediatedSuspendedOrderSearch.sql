CREATE PROCEDURE [dbo].[GetAllRemediatedSuspendedOrderSearch]
	@OrderHeader_ID					int,
...
AS 
-- **************************************************************************
-- Procedure: GetAllRemediatedSuspendedOrderSearch()
--    Author: n/a
--      Date: n/a
--
-- Description:
...
BEGIN
	...
	SELECT
		...
		[TotalInvoiceCostNoCharges] =	ISNULL(ov.InvoiceTotalCost, 0),
		[TotalInvoiceFreight]		=	ISNULL(ov.InvoiceFreight, 0),
		[TotalOrderCost]			=	CASE ISNULL(oh.DiscountType, -1)
											WHEN 1 THEN 
		...
		[OrderUnitMismatch]			=	Case 
											...
											Else 0
										End,
		[Charges]					=	ISNULL((SELECT SUM(Value) FROM OrderInvoiceCharges WHERE OrderHeader_Id = oh.OrderHeader_ID AND IsAllowance = 0), 0),										
	...
	FROM
		...
		-- TFS 5880    Added index to OrderItem to avoid seach timeout
		--INNER JOIN	dbo.OrderItem							(nolock) oi		ON	oh.OrderHeader_Id					= oi.OrderHeader_Id
		INNER JOIN  dbo.OrderItem oi WITH (INDEX(idxOrderItemIDHeaderID), nolock) ON oh.OrderHeader_Id  = oi.OrderHeader_Id
	...
	WHERE
		...
		AND ((@LocalEInvoiceOnly = 0 AND ISNULL(oh.eInvoice_Id, 0) =  ISNULL(oh.eInvoice_Id, 0))
			OR (@LocalEInvoiceOnly = 1 AND oh.eInvoice_Id is not null))
		-- Invoice Date search criteria
		--AND ((@LocalInvoiceDateStart IS NULL OR @LocalInvoiceDateEnd IS NULL)
		--	OR (@LocalInvoiceDateStart IS NOT NULL 
		--	AND @LocalInvoiceDateEnd IS NOT NULL
		...
		--AND ((@LocalOrderDateStart IS NULL OR @LocalOrderDateEnd IS NULL)
		--	OR (@LocalOrderDateStart IS NOT NULL 
		--		AND @LocalOrderDateEnd IS NOT NULL
		...
		AND ((@LocalResolutionCodeID = 0 and oh.ResolutionCodeID IS NOT NULL)
			...
			OR (@LocalResolutionCodeID = 0 and oi.ResolutionCodeID IS NOT NULL)
			OR (@LocalResolutionCodeID > 0 and oi.ResolutionCodeID = @LocalResolutionCodeID))	
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[GetAllRemediatedSuspendedOrderSearch] TO [IRMAClientRole]
    AS [dbo];

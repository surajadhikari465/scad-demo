CREATE PROCEDURE dbo.msqBakeryInventoryPonce
    ...
    SET Cost = (SELECT SUM(ReceivedItemCost + ReceivedItemFreight) / SUM(UnitsReceived)
                       ...
                       WHERE OrderHeader_ID = 
                             (SELECT TOP 1 OrderHeader.OrderHeader_ID
                              ...
                              WHERE VendStore.Store_No = 10196
                                  AND Return_Order = 0
...
WHERE Vendor.Vendor_ID IN (5527,  5319,  5062,  6000,  5255,  6354,  5004,  5139,  5572,  1165,  3573,  5596,  5116,  5243,  390,  6052,  5318,  5281,  3935,  5701,  586,  6035,  5058,  5793)     
AND SubTeam_No = 4200 AND Price.Store_No = 10196  	      

CREATE PROCEDURE dbo.msqBakeryInventoryCary
    ...
    SET Cost = (SELECT SUM(ReceivedItemCost + ReceivedItemFreight) / SUM(UnitsReceived)
                       ...
                       WHERE OrderHeader_ID = 
                             (SELECT TOP 1 OrderHeader.OrderHeader_ID
                              ...
                              WHERE VendStore.Store_No = 10132
                                  AND Return_Order = 0
...
WHERE Vendor.Vendor_ID IN (5319,5546,5972,140,5514,5596,5485,5968,5273,5868,5669,5671,5281,4997,6118,5732)    	      
AND SubTeam_No = 4200 AND Price.Store_No = 10132  	      

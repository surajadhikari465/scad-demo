...
IF  EXISTS (SELECT job_id FROM msdb.dbo.sysjobs_view WHERE name = N'ItemCatalog - ItemHistory Purge')
EXEC msdb.dbo.sp_delete_job @job_name=N'ItemCatalog - ItemHistory Purge', @delete_unused_schedule = 1
GO
...
/****** Object:  Job [ItemCatalog - ItemHistory Purge]    Script Date: 3/23/2017 6:31:04 AM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]    Script Date: 3/23/2017 6:31:04 AM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'ItemCatalog - ItemHistory Purge', 
		@enabled=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Check Maintenance Mode]    Script Date: 3/23/2017 6:31:04 AM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Check Maintenance Mode', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [ItemHistory Purge]    Script Date: 3/23/2017 6:31:04 AM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'ItemHistory Purge', 
		@step_id=2, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
		@on_success_step_id=0, 
		...
		@command=N' DECLARE @cutOffDate	
					...
					EXEC [dbo].[PurgeItemHistoryDaily] @cutOffDate, 50000', 
		@database_name=@DBNAME, 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Report Maintenance Mode]    Script Date: 3/23/2017 6:31:04 AM ******/

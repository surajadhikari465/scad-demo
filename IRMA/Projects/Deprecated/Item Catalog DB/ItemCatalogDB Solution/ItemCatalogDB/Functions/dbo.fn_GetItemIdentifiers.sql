-- NOTE: Dummy FN def must have same return type: table or scalar.
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[fn_GetItemIdentifiers]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
	EXEC ('CREATE FUNCTION [dbo].[fn_GetItemIdentifiers]()
	RETURNS @ItemIdentifier TABLE (Identifier_ID INT)
	begin
	return
	end')
GO

ALTER FUNCTION [dbo].[fn_GetItemIdentifiers] ()

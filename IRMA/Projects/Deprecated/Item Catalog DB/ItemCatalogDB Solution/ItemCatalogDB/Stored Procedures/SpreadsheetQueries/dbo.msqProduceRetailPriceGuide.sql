...
/****** Object:  StoredProcedure [dbo].[msqProduceRetailPriceGuide]    Script Date: 05/11/2006 12:03:37 ******/
IF  EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[msqProduceRetailPriceGuide]') AND OBJECTPROPERTY(id,N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[msqProduceRetailPriceGuide]
GO

/****** Object:  StoredProcedure [dbo].[msqProduceRetailPriceGuide]    Script Date: 05/11/2006 12:03:44 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
BEGIN
    ...
    SELECT 
        Identifier, Item_Description AS 'Item Description', Package_Desc1 AS 'Pack', Unit_Name AS 'Unit', 
      	CASE WHEN dbo.fn_OnSale(PriceChgTypeID) = 1 THEN Price.Sale_Multiple ELSE Price.Multiple END AS 'Mult', 
		dbo.fn_Price(PriceChgTypeID, Multiple, Price, PricingMethod_ID, Sale_Multiple, Sale_Price) AS 'Retail', 
		(dbo.fn_Price(PriceChgTypeID, Multiple, Price, PricingMethod_ID, Sale_Multiple, Sale_Price) * Package_Desc1) * CasePriceDiscount AS 'CaseRetail', 

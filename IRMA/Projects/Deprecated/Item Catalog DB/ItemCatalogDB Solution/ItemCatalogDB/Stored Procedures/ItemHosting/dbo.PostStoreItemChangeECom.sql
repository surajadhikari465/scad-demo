if not exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[PostStoreItemChangeECom]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
	EXEC('CREATE PROCEDURE [icon].[PostStoreItemChangeECom] AS BEGIN SET NOCOUNT ON; END')
GO

ALTER PROCEDURE dbo.PostStoreItemChangeECom
	@Item_Key int, 
    @Store_No int, 

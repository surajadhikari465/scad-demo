if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[UpdateItemInfo]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[UpdateItemInfo]
GO
...
-- 2016-08-19	JA			20599 (17483) Added Not available for 365 field 
-- **************************************************************************
...
BEGIN
	...
	-- check to see if there is a match in the item override 365 table
	SELECT @365RecordExists = 1 FROM ItemOverride365 WHERE Item_Key = @Item_Key
	...
	IF @365RecordExists is null and @Not_Available_365 = 1 	
		INSERT into ItemOverride365
		(Item_Key, Not_Available)
		Values
		(@Item_Key, @Not_Available_365)
	ELSE	
		UPDATE ItemOverride365 SET
		Not_Available = @Not_Available_365

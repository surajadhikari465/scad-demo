IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Replenishment_TagPush_GetElectronicShelfTagBatchFile]') AND type in (N'P', N'PC'))
begin
	EXEC ('CREATE PROCEDURE [dbo].[Replenishment_TagPush_GetElectronicShelfTagBatchFile] AS SELECT 1')
end
GO
...
ALTER PROCEDURE [dbo].[Replenishment_TagPush_GetElectronicShelfTagBatchFile]
AS
 --**************************************************************************************

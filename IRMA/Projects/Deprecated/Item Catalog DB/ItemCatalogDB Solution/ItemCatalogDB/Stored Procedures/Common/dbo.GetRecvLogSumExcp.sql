...
if not exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[GetRecvLogSumExcp]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	EXEC ('CREATE PROCEDURE [dbo].[GetRecvLogSumExcp] AS SELECT 1')
end
GO

ALTER PROCEDURE dbo.GetRecvLogSumExcp 
    @Store_No		int,
    @Begin			varchar(255),
    @End			varchar(255),
    @SubTeam_No		int

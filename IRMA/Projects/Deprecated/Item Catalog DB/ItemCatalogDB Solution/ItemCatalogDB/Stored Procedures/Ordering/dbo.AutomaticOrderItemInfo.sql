...
if not exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[AutomaticOrderItemInfo]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	EXEC ('CREATE PROCEDURE [dbo].[AutomaticOrderItemInfo] AS SELECT 1')
end
GO

ALTER PROCEDURE [dbo].[AutomaticOrderItemInfo]
	@Item_Key		int, 
	@OrderHeader_ID int, 

IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Planogram_GetAccessViaSetRegTagFile]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Planogram_GetAccessViaSetRegTagFile]
GO

CREATE PROCEDURE [dbo].[Planogram_GetAccessViaSetRegTagFile]
...
-- ****************************************************************************************************************
-- Procedure: Planogram_GetAccessViaSetRegTagFile()
--    Author: unknown
--      Date: unknown
--
--
...
BEGIN
    ...
    SELECT DISTINCT
		...
		[Organic] = I.Organic,
		[Discontinued] = dbo.fn_GetDiscontinueStatus(I.Item_Key, @Store_No, NULL),-- I.Discontinue_Item,											
      ...
      FROM 
		...
		JOIN dbo.StoreItem SI (nolock) ON ST.Store_no=SI.Store_No AND I.Item_Key=SI.Item_Key AND SI.Authorized = 1
	...
	BEGIN CATCH
		...
		RAISERROR ('Planogram_GetAccessViaSetRegTagFile failed with  @ErrorMessage' , -- Message text.

...
const InvoiceData: React.FC<IProps> = ({ match }) => {
    ...
    const [invoiceDate, setInvoiceDate] = useState<string>(dateFormat(new Date(), 'yyyy-mm-dd'));
            ...
            if(orderDetails) {
                switch(radioSelection) { 
                    case radioOptions.Invoice:
                        if(orderDetails.InvoiceDate && orderDetails.InvoiceDate !== '0001-01-01T00:00:00') {
                    ...
                    case radioOptions.Other:
                        ...
                        if(orderDetails.VendorDocId){
                            setInvoiceNumber(orderDetails.VendorDocId);
                        }
                        break;
                }
                ...
                if(!invoiceTotalEdited) {
                    ...
                    if(orderDetails.OrderTypeId === 3 || orderDetails.OrderTypeId === 2) {
                        ...
                        setInvoiceTotal(parseFloat((orderDetails.InvoiceCost + nonAllocatedCharges).toFixed(2)));
                    } else {
    ...
    }, [charges, radioOptions, orderDetails, radioSelection, region, invoiceTotalEdited]); 
    ...
    const handleReparseEInv = async () => {
        ...
        try {
            ...
            if(result.status) {
                ...
                toast.error(`Error when reparsing eInvoice: ${result.errorMessage}`)
            }
    ...
    return (
        <Fragment>
                ...
                <div style={{backgroundColor: 'lightgrey', border: '1px solid black'}}>
                    <Form.Group inline>
                        ...
                        <Form.Field ><label style={{textAlign: 'right', width: '80px'}}>Date:</label> <Input disabled={(orderDetails?.EInvRequired) || radioSelection === radioOptions.None} max={dateFormat(new Date(), "yyyy-mm-dd")} 
                                    ...
                                    value={radioSelection === radioOptions.None ? "" : dateFormat(invoiceDate, 'yyyy-mm-dd')}
            ...
            <div style={{position: 'absolute', bottom: '5px', width: '100%', textAlign: 'center'}}>
                <ConfirmModal triggerButtonText="Close Invoice" handleConfirmClose={closeOrder} lineOne={`Invoice Date: ${dateFormat(new Date(invoiceDate), 'mm/dd/yyyy')}`} 

...
Friend Class frmPricingPrintSigns
    ...
    Private Sub cmdPrint_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdPrint.Click
        ...
        If SlawPrintBatchConfigurationDAO.SlawPrintRequestsEnabledForRegion() Then
                ...
                If selectedItems.Any(Function(i) i.IdentifierIsValid = True) Then
                    ...
                    Catch ex As Exception
                        ...
                        If response IsNot Nothing Then
                            ...
                            ' add the api response error to our list, but do not log or alert until all requests have processed
                            slawApiErrorResponses.Add(String.Format("Error in sending ad-hoc request to SLAW API:{0}{1}.{0}SLAW Response:{0}{2}{0}",
                                   Environment.NewLine, ex.Message, slawJsonResponseText))
                        End If

                        ' add the batch/store/subteam info for the failed print request to our list of errors for later display
                        Dim failedRequest As New PriceBatchFailedPrintRequestsBO With {.BatchName = batchName, .StoreNumber = storeNumber, .SubteamName = selectedSubteamName}
                        failedAdHocPrintRequests.Add(failedRequest)
                    Finally
                        Cursor = Cursors.Default
                    End Try
            ...
            'communicate any compiled errors
            If (failedAdHocPrintRequests.Count > 0) Then
                ' combine the list of failed requests into a single string for display & logging
                Dim concatenatedPrintRequestErrors As String = failedAdHocPrintRequests _
                    .Select(Function(f) String.Format("{0}: Store {1} - {2}{3}", f.BatchName, f.StoreNumber, f.SubteamName, Environment.NewLine)) _
                    .Aggregate(Function(a, b) a + b)
                ' log all the failed print batch requests together
                logger.Info(String.Format("Failed print batch requests:{0}{1}", Environment.NewLine, concatenatedPrintRequestErrors))

                ' also combine the list of api error responses into a single list
                Dim concatenatedApiErrorResponses As String = String.Empty
                If (slawApiErrorResponses.Count > 0) Then
                    concatenatedApiErrorResponses += String.Join(Environment.NewLine, slawApiErrorResponses)
                    ' log all the API responses  together
                    logger.Info(concatenatedApiErrorResponses)
                End If

                ' alert the user to the errors, and include the details from the failed batch requests and response info in a single pop-up
                Dim errorSummary As String = "The following ad-hoc print requests failed to process. This information has also been written to the local IRMA client log file."
                FailedRequestsDialog.HandleError(errorSummary, concatenatedApiErrorResponses, concatenatedPrintRequestErrors)

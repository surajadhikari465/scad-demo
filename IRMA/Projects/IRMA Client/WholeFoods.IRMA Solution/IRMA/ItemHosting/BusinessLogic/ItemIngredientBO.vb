Namespace WholeFoods.IRMA.ItemHosting.BusinessLogic
    Public Class ItemIngredientBO
        Private _itemNutritionId As Integer
        Private _scale_Ingredient_ID As Integer
        Private _description As String
        Private _ingredients As String

#Region "Property Access Methods"
        Public Property ItemNutritionId As Integer
            Get
                Return _itemNutritionId
            End Get
            Set(value As Integer)
                _itemNutritionId = value
            End Set
        End Property
        Public Property ScaleIngredientID() As Integer
            Get
                Return _scale_Ingredient_ID
            End Get
            Set(ByVal value As Integer)
                _scale_Ingredient_ID = value
            End Set
        End Property

        Public Property Description() As String
            Get
                Return _description
            End Get
            Set(ByVal value As String)
                _description = value
            End Set
        End Property
        Public Property Ingredients() As String
            Get
                Return _ingredients
            End Get
            Set(ByVal value As String)
                _ingredients = value
            End Set
        End Property
#End Region
    End Class
End Namespace

...
Friend Class frmReceivingList
	...
	Private Sub PopGrid(Optional ByRef bRefreshData As Boolean = True)
		...
		Call SetActive(dtpPastReceiptDate, (Not bTransferOrder) And (Not bReturnOrder) And (Not bAtLeastOneReceived) And (dtpPastReceiptDate.Value Is Nothing) And (chkPastReceipt.Checked), False)
	...
	Private Sub cmdReceive_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdReceive.Click
		...
		If m_bAllowReceiveAll = False Then
			...
			If cellWithNoReceivedQuantity Then
				grdRL.Selected.Rows.Clear()
				MsgBox("Please enter a received quantity for each line.", MsgBoxStyle.OkOnly +
		...
		For iLoop = 0 To nTotalSelRows - 1
			...
			If FlagCostedByWeight And
				CBool(grdRL.Selected.Rows(iLoop).Cells("isPackageUnit").Text) And
				Not FlagCatchWeightRequired _
				And Val(sReceivedValue) > 0 And
				Not FlagEinvoiceQtyExists And
			...
			Else
				...
				ElseIf sReceivedValue <> "NULL" AndAlso EInvoice_Id <> -1 AndAlso grdRL.Selected.Rows(iLoop).Cells("Code").Text = "" AndAlso
		...
		If frmOrders.ItemsReceived And dtpPastReceiptDate.Enabled Then
			...
			ElseIf bShowInvoiceExceptions Then
				If Not row.Cells("eInvoice").Value.Equals(0) Then
					...
					Or (Not row.Cells("eInvoice Unit").Value Is DBNull.Value _
						And Not row.Cells("eInvoice Unit").Value.Equals(row.Cells("Unit").Value)
					) Then
						bShow = True
					Else
						bShow = False

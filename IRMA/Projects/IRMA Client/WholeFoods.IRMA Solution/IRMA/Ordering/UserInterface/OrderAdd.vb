...
Friend Class frmOrderAdd
                    ...
                    If (Global_Renamed.geOrderType = Global_Renamed.enumOrderType.Transfer) Then
                        ...
                        If (m_bIsVendorDistributorManafacturer) Then
                            If (VB6.GetItemData(cmbField(iOrderHeaderReceiveLocation_ID), cmbField(iOrderHeaderReceiveLocation_ID).SelectedIndex) = glVendorID) Then
                                Call LoadSubTeamByType(Global_Renamed.enumSubTeamType.StoreByVendorID, cmbField(2), m_abSubTeam_From_UnRestricted, glVendorID, 0)
                            Else
                                Call LoadSubTeamByType(Global_Renamed.enumSubTeamType.StoreMinusSupplier, cmbField(2), m_abSubTeam_From_UnRestricted, glStoreNo, 0)
                            End If
                        End If
                    End If
    ...
    Private Sub PopulateTransferToSubTeam()
        ...
        If (Global_Renamed.geOrderType = Global_Renamed.enumOrderType.Purchase Or Global_Renamed.geOrderType = enumOrderType.Flowthru) And (lReceivingVendorID > 0) Then
            ...
            If (DeterminePurchaserFacility(lReceivingVendorID) = True) Then
                If Global_Renamed.geProductType = enumProductType.OtherSupplies Then
                    ...
                    Call LoadSubTeamByType(Global_Renamed.enumSubTeamType.Supplies, cmbField(3))
                    ' load the SupplyTranserToSubTeam (labeled "Transfer To") with only "other supplies" subteams
                    Call LoadSubTeamByType(Global_Renamed.enumSubTeamType.Supplies, cmbField(4))
                Else
                    Call LoadSubTeamByType(Global_Renamed.enumSubTeamType.All, cmbField(3), m_abSubTeam_To_UnRestricted, lReceivingVendorID, 0)
                End If
            Else
                'Load the purchasing store's subteam list from the StoreSubteam table
                If Global_Renamed.geProductType = enumProductType.OtherSupplies Then
                    ...
                    Call LoadSubTeamByType(Global_Renamed.enumSubTeamType.All, cmbField(4))
                Else
                    Call LoadVendStoreSubteam(cmbField(3), lReceivingVendorID, m_abSubTeam_To_UnRestricted)
                End If
            End If
        ElseIf (lReceivingVendorID > 0) Then
            ...
            If Global_Renamed.geProductType = enumProductType.OtherSupplies Then
                ' if Other supplies, load the Transfer To (labeled "Supply Type") with only "other supplies" subteams
                Call LoadSubTeamByType(Global_Renamed.enumSubTeamType.Supplies, cmbField(3), m_abSubTeam_To_UnRestricted)
                ' load the SupplyTranserToSubTeam (labeled "Transfer To") with All subteams
                Call LoadSubTeamByType(Global_Renamed.enumSubTeamType.All, cmbField(4), m_abSubTeam_To_UnRestricted)
            Else
                Call LoadVendStoreSubteam(cmbField(3), lReceivingVendorID, m_abSubTeam_To_UnRestricted)
            End If
        End If
    ...
    Private Sub DisplayTransfer()
        ...
        Else
            ...
            Call LoadSubTeamByType(Global_Renamed.enumSubTeamType.Store, cmbField(2), m_abSubTeam_From_UnRestricted, glStoreNo, 0)
        End If
    ...
    Private Sub DisplayDistribution()
        ...
        Call LoadSubTeamByType(Global_Renamed.enumSubTeamType.Supplier, cmbField(2), m_abSubTeam_From_UnRestricted, glStoreNo, 0)

...
Friend Class frmOrderItemQueue
	...
	Private Sub cmbSubteam_KeyPress(ByVal sender As Object, ByVal eventArgs As KeyPressEventArgs) Handles cmbTransferToSubteam.KeyPress, cmbTransferFromSubteam.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		If KeyAscii = ASCII_BACKSPACE Then
	...
	Private Sub DisplayDistribution()
		...
		Call LoadSubTeamByType(enumSubTeamType.Supplier, cmbTransferFromSubteam, m_abSubTeam_From_UnRestricted, glStoreNo, 0, chkSubteam.Checked)
	...
	Private Sub DisplayTransfer()
		...
		If m_bIsVendorDistributorManufacturer Then
			Call LoadSubTeamByType(enumSubTeamType.StoreMinusSupplier, cmbTransferFromSubteam, m_abSubTeam_From_UnRestricted, glStoreNo, 0, chkSubteam.Checked)
		Else
			' Change From-Subteam to storeSubTeams
			Call LoadSubTeamByType(enumSubTeamType.Store, cmbTransferFromSubteam, m_abSubTeam_From_UnRestricted, glStoreNo, 0, chkSubteam.Checked)
		End If
	...
	Private Sub cmbPurchasing_SelectedIndexChanged(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmbPurchasing.SelectedIndexChanged
		...
		If (geOrderType = enumOrderType.Purchase) And (lPurchasingID > 0) Then
			...
			If (DeterminePurchaserFacility(lPurchasingID) = True) Then
				Call LoadSubTeamByType(enumSubTeamType.SupplierByVendor, cmbTransferToSubteam, m_abSubTeam_To_UnRestricted, lPurchasingID, 0, chkSubteam.Checked)
			Else
				'Load the purchasing store's subteam list from the StoreSubteam table
				LoadSubTeamByType(enumSubTeamType.StoreByVendorID, cmbTransferToSubteam, m_abSubTeam_To_UnRestricted, lPurchasingID, isIncludeAllTeams:=chkSubteam.Checked)
			End If
	...
	Private Sub chkFilterSubTeam_MouseClick(sender As Object, e As MouseEventArgs) Handles chkSubteam.MouseClick
		RefreshSubteamCombo(cmbTransferFromSubteam, m_abSubTeam_From_UnRestricted, chkSubteam.Checked)
		RefreshSubteamCombo(cmbTransferToSubteam, m_abSubTeam_To_UnRestricted, chkSubteam.Checked)
	End Sub
End Class
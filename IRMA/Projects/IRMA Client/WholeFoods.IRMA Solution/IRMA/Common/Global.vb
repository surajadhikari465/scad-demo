...
#Region "Public methods to load combos"
	...
	Public Sub LoadSubTeamByType(ByRef eSubTeamType As enumSubTeamType, ByRef cmbSubTeam As ComboBox, Optional ByRef lNumber As Integer = -1, Optional ByRef lOrderHeader_SubTeamNo As Integer = -1)
	...
	Public Sub LoadSubTeamByType(ByRef eSubTeamType As enumSubTeamType, ByRef cmbSubTeam As ComboBox, ByRef abSubTeamUnRestricted() As Boolean, Optional ByRef lNumber As Integer = -1, Optional ByRef lOrderHeader_SubTeamNo As Integer = -1, Optional ByVal isIncludeAllTeams As Boolean = True)
		Dim paramList As New DBParamList()
		Dim storedProcedure As String = String.Empty

		Try
			cmbSubTeam.Items.Clear()
			cmbSubTeam.Tag = Nothing
			...
			Dim factory As New DataFactory(DataFactory.ItemCatalog)
			Dim subTeams As List(Of SubTeamBO) = factory.GetStoredProcedureDataTable(storedProcedure, paramList) _
				.Rows.Cast(Of DataRow) _
				.Select(Function(x) New SubTeamBO() With {
						.SubTeamNo = CInt(x!SubTeam_No),
						.SubTeamName = x!SubTeam_Name.ToString(),
						.IsUnrestricted = CBool(x!SubTeam_Unrestricted),
						.IsDisabled = CBool(If(IsDBNull(x!IsDisabled), False, x!IsDisabled)),
						.AlignedSubTeam = CBool(If(IsDBNull(x!AlignedSubTeam), False, x!AlignedSubTeam))}) _
				.ToList()

			cmbSubTeam.Tag = subTeams 'Store all Subteams to avoid multiple DB calls when filtering inactive ones.
			RefreshSubteamCombo(cmbSubTeam, abSubTeamUnRestricted, isIncludeAllTeams)
		Catch ex As Exception
			cmbSubTeam.Tag = Nothing
			logger.Error(ex.Message)
			Throw ex

IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'GetItemLocaleChanges')
	EXEC('CREATE PROCEDURE [mammoth].[GetItemLocaleChanges] AS BEGIN SET NOCOUNT ON; END')
GO

ALTER PROCEDURE [mammoth].[GetItemLocaleChanges]
	@JobInstance int
AS
BEGIN
	SELECT

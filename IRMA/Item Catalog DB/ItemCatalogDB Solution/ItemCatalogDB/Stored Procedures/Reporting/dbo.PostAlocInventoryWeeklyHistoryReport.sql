...
/****** Object:  StoredProcedure [dbo].[PostAlocInventoryWeeklyHistoryReport]    Script Date: 10/04/2012 15:49:45 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[PostAlocInventoryWeeklyHistoryReport]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[PostAlocInventoryWeeklyHistoryReport]
GO
...
CREATE PROCEDURE [dbo].[PostAlocInventoryWeeklyHistoryReport]
    @Store_No	int,
	@SubTeam_No int
WITH RECOMPILE
AS
   -- **************************************************************************
   -- Procedure: PostAlocInventoryWeeklyHistoryReport()
   --    Author: n/a
   --      Date: n/a
   --
   -- Description:
   ...
   --						removed roll-forward addition of Allocated values;
   ...
   --						EXEC PostAlocInventoryWeeklyHistoryReport 40808, 4
...
SET NOCOUNT ON
	...
	--**************************************************************************
	-- Get Daily item qty modifiers based upon Allocations from OrderHeader
	--**************************************************************************
	DECLARE @ModAllocated TABLE 
			(
			ModItem		int,
			ModDate		datetime,
			ModQty		int,
	...
	INSERT INTO @ModAllocated
		SELECT
			oi.Item_key						AS ModItem,
			oh.Expected_Date				AS ModDate,
			SUM(oi.QuantityAllocated) * -1	AS ModQty,
			datename(dw, dateadd("d", -1, oh.Expected_Date))  AS ModDay
		FROM
			OrderItem				(nolock) oi
			INNER JOIN OrderHeader	(nolock) oh	ON oh.Orderheader_ID	= oi.Orderheader_ID
	...
	--**************************************************************************
	-- Merge Send/Orders and Allocated to come up with total daily modifiers
	--**************************************************************************
	...
	INSERT INTO @ModQty
		SELECT
			ModItem,
			ModDay,
			SUM(ModQty) AS ModQty
		FROM
			(
			SELECT * FROM @ModOrder
			UNION 
			SELECT * FROM @ModAllocated
			) AS Mods
		GROUP BY 	
			ModItem,
			ModDay
	...
	FROM
		...
		GROUP BY
			ModItem
		) AS alo ON tot.Item_Key = alo.ModItem
		LEFT OUTER JOIN 
		(
		SELECT
			ModItem,
	...
	SELECT * FROM @tmpRecords 
		WHERE 
		(MonAL > 0 OR TueAL > 0 OR WedAL > 0 OR ThuAL > 0 OR FriAL > 0 OR SatAL > 0 OR SunAL > 0)
		OR

IF NOT EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[Replenishment_POSPush_PopulateIconPOSPushPublish]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
	EXEC ('create PROCEDURE [dbo].[Replenishment_POSPush_PopulateIconPOSPushPublish] (@foo int) as select 1')
GO

ALTER PROCEDURE [dbo].[Replenishment_POSPush_PopulateIconPOSPushPublish]
	@ApplyChanges INT = 0
AS
BEGIN
-- ******************************************************************************************************************************************************************************************************************************

IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[UpdateStoreItem]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[UpdateStoreItem]
GO

CREATE PROCEDURE dbo.UpdateStoreItem 
    @Item_Key		int, 
    @Store_No		int, 
	@AuthorizedItem bit,
	@Refresh		bit
...
/***************************************************************************
Procedure:		UpdateStoreItem
Author:			The South
Date:			When the earth was young
IRMA Version:	All of them
...
BEGIN TRAN
    ...
    ELSE    
		BEGIN    
			...
			SET NOCOUNT OFF    
			RAISERROR ('UpdateStoreItem failed with @@ERROR: %d', @Severity, 1, @Error_No)           
		END   
END
GO 
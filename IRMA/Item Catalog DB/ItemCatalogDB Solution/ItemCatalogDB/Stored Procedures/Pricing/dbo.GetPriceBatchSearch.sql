...
IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'dbo.GetPriceBatchSearch') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[GetPriceBatchSearch]
GO
...
--------------------------------------------------------------------------------------------------------
	...
	SELECT TOP(@MaxRows)
		...
		[SaleEndDate] = CASE WHEN MAX(CASE WHEN PBD.Sale_End_Date IS NULL THEN 0 ELSE 1 END)=1 THEN MAX(PBD.Sale_End_Date) END,
	...
	WHERE 
		...
		AND ISNULL(PBH.StartDate, 0) BETWEEN COALESCE(@FromStartDate, PBH.StartDate, 0) AND COALESCE(@ToStartDate, PBH.StartDate, 0)
		 --PriceChgTypeId
		AND (ISNULL(PBH.PriceChgTypeID, 0) = 
			CASE 
				WHEN ISNULL(@ItemChgTypeID, 0) = 5 
				THEN ISNULL(PBH.PriceChgTypeID, 0) 
				WHEN ISNULL(@PriceChgTypeID, 0) = 0
				THEN ISNULL(PBH.PriceChgTypeID, 0)
				ELSE ISNULL(@PriceChgTypeID, 0) 
			END)		
		--itemchgtype

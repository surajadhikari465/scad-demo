...
namespace OOS.Model.Repository
{
    public class RetailItemRepository : IRetailItemRepository
    {
        ...
        public List<IRetailItem> For(IEnumerable<string> upcsQueried, string storeAbbrev)
        {
            ...
            var ps_bu = storeRepository.ForAbbrev(storeAbbrev);
            string queryOracle =
                ...
                "WHERE ir.upc in ({0}) AND vsi.ps_bu = '" + ps_bu.BusinessUnitNo + "' " +
                ...
                if ((ix + 1) >= upcs.Count || upcList.Length >= flushLength)
                {
                    ...
                    using (var dc = new System.Data.Linq.DataContext(config.GetOOSConnectionString()))
                    {
                        ...
                        foreach (var item in resultInner)
                        {
                            ...
                            var subTeamName = item.SUBTEAM_NAME;
                            ...
                            var retailItem = new RetailItem(productMap[upc])
                                                 {
                                                     VendorKey = vendorKey, VendorItemNumber = vin, TeamName = teamName, SubTeamName = subTeamName, 
                                                     PriceType = priceType, PepoleSoftBusinessUnit = businessUnit, CaseSize = caseSize, Cost = cost, Price = price   
                                                 };

 ...
 */
 ...
 (function($) {
	...
	**/
	...
	/* Get a selected dated from a picker */
	...
	**/
	...
	var getFormattedDate = function(date, date_format) {
		...
		var currentHour = hou;
		var is_AM = hou < 12 || (hou == 12 && min == 0);
	    var isAMText = is_AM ? " AM" : " PM";

		if (!is_AM) currentHour = currentHour - 12;
		if (is_AM && currentHour == 0) currentHour = 12;
		if (!is_AM && currentHour == 0) currentHour = 12;
		console.log(currentHour + " " + is_AM);
	    ...
	    date_format = date_format.replace(/YYYY/gi, y)
	        ...
	        .replace(/D/g, d)
	        .replace(/hh/g, zpadding(currentHour))
	        .replace(/h/g, currentHour)
	        .replace(/mm/g, zpadding(min))
	        .replace(/m/g, min)
	        .replace(/aa/g, isAMText);
		return date_format;
	};
	...
	var draw = function($picker, option, year, month, day, hour, min) {
		...
		if ($picker.data("dateOnly") === true) {
			...
			while( hour_*100+min_ < maxTime[0]*100+maxTime[1] ){
			    ...
			    var is_AM = hour_ < 12 || (hour_ == 12 && min_ == 0);

				
				$o.addClass('timelist_item');

				var currentHour = hour_;
				if (!is_AM) currentHour = currentHour - 12;
				if (is_AM && currentHour == 0) currentHour = 12;
				if (!is_AM && currentHour == 0) currentHour = 12;
			    console.log(currentHour + " " + is_AM);
			    ...
			    if (is_AM)
			        $o.text(zpadding(currentHour) + ":" + zpadding(min_) + " AM");
                else
			        $o.text(zpadding(currentHour) + ":" + zpadding(min_) + " PM");

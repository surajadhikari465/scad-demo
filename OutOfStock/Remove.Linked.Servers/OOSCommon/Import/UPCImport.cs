...
{
    public class UPCImport : IOOSImportReported
    {
        ...
        public UPCImport(DateTime? scanDate, OOSCommon.DataContext.STORE store, bool isValidationMode,
        ...
        protected bool ImportNextBatch(string filePath, System.IO.StreamReader fileStream)
        {
            ...
            this.recordCount = 1;
            this.itemCount = 0;
            if (scanDate.HasValue && outputData.BeginBatch(scanDate.Value))
            {
                //outputData.BeginBatch(scanDate.Value);
                // Get upc's

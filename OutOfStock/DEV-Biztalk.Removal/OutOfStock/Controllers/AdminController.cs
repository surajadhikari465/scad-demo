...
namespace OutOfStock.Controllers
{
    public class AdminController : Controller
    {
        ...
        public AdminController(IUserLoginManager loginManager)
        {
        ...
        public JsonResult SaveLocationOverride(SaveLocationOverride data)
        {
            ...
            try
            {
                ...
                {
                    if (!string.IsNullOrEmpty(data.Region) || !string.IsNullOrEmpty(data.Store))
                    {
                        var region = db.REGION.FirstOrDefault(r => r.REGION_ABBR == data.Region);
                        if (region == null) throw new AdminExceptionRegionNotFound(data.Region);
                        var store = db.STORE.FirstOrDefault(s =>
                            s.STORE_ABBREVIATION == data.Store && s.REGION_ID == region.ID);
                        if (store == null) throw new AdminExceptionStoreNotFound(data.Region, data.Store);
                    }

                    var user = db.Users.FirstOrDefault(u => u.Username == data.Username);
                    if (user == null) throw new AdminExceptionUserNotFound(data.Username);
                    user.RegionOverride = string.IsNullOrEmpty(data.Region) ? null : data.Region.ToUpper();
                    user.StoreOverride = string.IsNullOrEmpty(data.Store) ? null : data.Store.ToUpper();
                    db.SaveChanges();

                    returnMessage = "ok";
                }
            }
            catch (AdminExceptionRegionNotFound regionNotFound)
            {
                returnMessage = regionNotFound.Message;
            }
            catch (AdminExceptionStoreNotFound storenotfound)
            {
                returnMessage = storenotfound.Message;
            }
        ...
        public JsonResult GetUserInfo(string user)
        {

            List<UserInfo> formattedUsers;
            using (var db = new OOSEntities())
            {
                var users = (from u in db.Users
                    where u.Username.Contains(user)
                    select u).OrderBy(u=>u.Username).Take(10).ToList();

                formattedUsers = users.Select(u => new UserInfo()
                {
                    Region = u.Region, 
                    RegionOverride = u.RegionOverride, 
                    Store = u.Store, 
                    StoreOverride = u.StoreOverride, 
                    Username = u.Username, 
                    LastLogin = u.LastLogin.ToString("MM/dd/yyyy hh:mm:ss tt")
                }).ToList();

            }

            return Json(formattedUsers, JsonRequestBehavior.AllowGet);
        }
...
";
    ...
    public class UserInfo
    {
        ...
        {
            get { return Username.ToLower().Replace(".", "").Replace("\\", ""); }
        }
    }

    public class SaveLocationOverride
    {
        public string Username { get; set; }
        public string Region { get; set; }
        public string Store { get; set; }
    ...
    public class AdminExceptionRegionNotFound : Exception
    {
        
        private readonly string _region;

        public AdminExceptionRegionNotFound(string region)
        {
            
            _region = region;
        }

        public override string Message => $"region {_region} not found.";
    }

    public class AdminExceptionStoreNotFound : Exception
    {
        private readonly string _region;
        private readonly string _store;

        public AdminExceptionStoreNotFound(string region, string store)
        {
            _region = region;
            _store = store;
        }

        public override string Message => $"store {_store} not found in region {_region}.";
    }
    public class AdminExceptionUserNotFound : Exception
    {
        private readonly string _username;

        public AdminExceptionUserNotFound(string username)
        {
            _username = username;
            
        }
        public override string Message => $"user {_username} not found.";
    }
}

...
namespace OutOfStock.Controllers
{
    public class CustomReportController : Controller
    {
        ...
        public CustomReportController(IUserProfile userProfile, IConfigurator config, IOOSEntitiesFactory entityFactory, IUserLoginManager loginManager)
        {
        ...
        public ActionResult Index()
        {
            ...
            logger.Trace($"CustomReportController::Index() Central: {userProfile.IsCentral().ToString()} Regional: {userProfile.IsRegionBuyer().ToString()} Region: {userProfile.UserRegion().ToString()} Store: {userProfile.UserStoreAbbreviation().ToString()}");
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        if (userProfile.IsCentral())
                        {
                            ...
                            {
                                regions = dbContext.REGION
                                    .Where(r => r.IS_VISIBLE.Equals("true", StringComparison.OrdinalIgnoreCase))
                                    .OrderBy(r => r.REGION_ABBR)
                                    .Select(r => r).ToList();
                            }
                            
                            ViewBag.MyRegions = regions;

                            // these will be filled in when the region is selcted on the UI.
                            ViewBag.MyStore = null;
                            ViewBag.MyStores = new List<SelectListItem>();
                        }
                        else
                        {
                            var currentRegion = userProfile.UserRegion();
                            using (var dbContext = entityFactory.New())
                            {
                                regions = dbContext.REGION
                                    .Where(r => r.IS_VISIBLE.Equals("true", StringComparison.OrdinalIgnoreCase) && r.REGION_ABBR == currentRegion)
                                    .OrderBy(r => r.REGION_ABBR)
                                    .Select(r => r).ToList();
                            }
                    ...
                    // Get teams from VIM 
                    List<KeyValuePair<string, string>> teamList = MvcApplication.vimRepository.GetVimTeam(regions)
                        .Where(t => !string.IsNullOrEmpty(t.team_name))
                        .OrderBy(d => d.team_name)
                        .Select(t => new { Encoded=WebUtility.HtmlEncode(t.team_name), Normal=t.team_name})
                        .Select(t => new KeyValuePair<string, string>(t.Encoded,t.Normal))
                        .ToList();
                    teamList.Insert(0, new KeyValuePair<string, string>(allSelectKey, WebUtility.HtmlEncode(allTeamsText)));
        ...
        public JsonResult LoadStoresByRegion(string regionId)
        {
            ...
            if (!string.IsNullOrWhiteSpace(regionId) && int.TryParse(regionId, out var regionIdAsInt))
        ...
        public JsonResult LoadSubTeamsByTeam(string regionName, string storeNumbers, string teamNames)
        {
            ...
            {
                string[] storeNumbersArray = storeNumbers.Split(new char[] { ',' })
                    .Where(s=> !string.IsNullOrEmpty(s))                    
                    .ToArray();


                isAllStoresSelected = storeNumbersArray.Contains(allSelectKey);
                ...
                {
                    List<string> storesForRegion;
                    using (var db = new OOSEntities())
                    {
                        storesForRegion = (from s in db.STORE
                            join r in db.REGION on s.REGION_ID equals r.ID
                            where r.REGION_ABBR == regionName && s.STATUS_ID == 3
                            select s.PS_BU).ToList();
                    }
                    storeHash.AddRange(storesForRegion);
                }
                else
                {
                    ...
                    using (var db = new OOSEntities())
                    {
                        ...
                        //convert storeNumbersArray to integers
                        var storeNumbersIntArray = storeNumbersArray.Select(int.Parse).ToArray();

                        businessUnits = (from s in db.STORE where storeNumbersIntArray.Contains(s.ID) select s.PS_BU).ToList();
                        storeHash.AddRange(businessUnits);
                    }
                }
            ...
            string selectedStartDate, string selectedEndDate, string clientTodayDate, int isGroupedByUpc)
        {
            ...
            logger.Trace($"CustomReportController::Index() Central: {userProfile.IsCentral()} Regional: {userProfile.IsRegionBuyer()} Region: {userProfile.UserRegion()} Store: {userProfile.UserStoreAbbreviation()}");
            ActionResult result = null;
            if (!userProfile.IsStoreLevel())
                result = new RedirectResult("~/Home/Index");
            else
        ...
        public static int TempFileCleanup()
        {
            ...
            try
            {
                ...
                foreach (var file in files.Where(f => f.CreationTime < DateTime.Now.AddHours(-1)))
                {
                    try
                    {
                        file.Delete();

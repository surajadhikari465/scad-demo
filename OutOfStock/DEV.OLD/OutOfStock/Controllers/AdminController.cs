    ...
    public class AdminController : Controller
    {
            ...
            using (var db = new OOSEntities())
            {
                try
                {
                    ...
                    if (existingStore == null)
                    {
                        ...
                        {
                            isError = true,
                            message = string.Format("Cannot find a store with Id [{0}]", store.StoreId)
                        };
                    }
                    else
                    {
            ...
            using (var db = new OOSEntities())
            {
                try
                {
                    ...
                    if (existingStore == null)
                    {
                        ...
                        {
                            isError = true,
                            message = string.Format("StoreId [{0}] does not exist for the selected region.", store.StoreId)
                        };
                    }
                    else
                    {
                        ...
                        switch (existingStore.STATUS_ID)
                        {
                            case 4:
                                ...
                                {
                                    isError = false,
                                    message = string.Format("[{0}] has been set to OPEN", store.StoreId)
                                };
                                break;
                            case 3:
                                ...
                                {
                                    isError = false,
                                    message = string.Format("[{0}] has been set to CLOSED", store.StoreId)
                                };
                                break;                            
                        }
            ...
            using (var db = new OOSEntities())
            {
                ...
                try
                {
                    ...
                    if (existingStore != null)
                    {
                        ...
                        {
                            isError = true,
                            message = string.Format("{0} [{1}] already exists in the database for region {2}",existingStore.PS_BU, existingStore.STORE_NAME, existingStore.REGION_ABBR)
                        };
                    ...
                    else
                    {
                        ...
                        {
                            isError = false,
                            message = string.Format("{0} [{1}] added.", newStore.PS_BU, newStore.STORE_NAME)
                        };
        ...
        protected IEnumerable<SelectListItem> GetRegions()
        {
            using (var dbContext = new OOSEntities())
            {
                ...
                return storeQuery.ToList().Select(region => new SelectListItem() { Text = string.Format("{0}", region.REGION_ABBR), Value = region.ID.ToString(CultureInfo.InvariantCulture) }).ToList();
            }
        }
        ...
        public static SageCall GetStoresByRegion(string Region)
        {
            ...
            var tokenURL =
                String.Format(
                    "{0}token?client_id={1}&grant_type=client_credentials&client_secret={2}&response_content_type=application/json",
                    url, publicKey, secret);
            ...
            var queryURL = String.Format("{0}{1}?access_token={2}&limit=1000", storeURL, Region, tokenResponse);

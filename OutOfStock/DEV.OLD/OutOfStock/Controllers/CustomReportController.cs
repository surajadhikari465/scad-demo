       ...
       // private IMovementRepository movementRepository;
            ...
            string selectedStartDate, string selectedEndDate, string clientTodayDate)
        {
            ...
            else
            {
                ...
                //// Fill in region, store, etc based on user permissions. defaults are set for certain user types.
                //if (!userProfile.IsCentral())
                //{
                //    selectedRegion = userProfile.UserRegion();
                ...
                //        if (storeId != 0) // Non Regional Admin user with a Valid Store. (not a regional office user)
                //            selectedStores = (storeId.HasValue ? storeId.Value.ToString(CultureInfo.InvariantCulture) : string.Empty);    

                //        // Non Regional Admin users that are in a Regional Office (not a store) will keep the stores they selected.
                ...
                try
                {
                    ...
                    telemetry.TrackEvent("CustomReport", properties, metrics);
                    ...
                    reportData = crvm.RunQuery(startDate, endDate, StoreInfo, TeamInfo, SubTeamInfo, todaysDate, ref entityFactory);

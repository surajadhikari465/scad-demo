        ...
        public IEnumerable<ICustomReportExcelModel> RunQuery(RunQueryParameters parameters, ref IOOSEntitiesFactory dbfactory )
        {
            ...
            using (var dbContext = dbfactory.New())
            {
                ...
                using (var datacontext = new OOSEntities(MvcApplication.oosEFConnectionString))
                {
                    ...
                    var multiStoreReportProcedures = new[] { "MultiStoreReport_NoGrouping", "MultiStoreReport_GroupByUpc" };

                    const string sql = @"EXEC dbo.{6} @Region = '{0}', -- varchar(5)
                    ...
                    var query = string.Format(sql, regionabbr, startDate, endDate, stores, teams, subTeams, multiStoreReportProcedures[groupByUpc]);

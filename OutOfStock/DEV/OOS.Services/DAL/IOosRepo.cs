    ...
    public class OosRepository : IOosRepo
    {
        ...
        public void SaveNewStores(List<StoreDb> stores)
        {
            ...
            var statement =
                ...
                + "(@PS_BU, @STORE_ABBREVIATION, @STORE_NAME, @REGION_ID, @STATUS_ID, 'StoreSync', @rightNow, 'StoreSync', @rightNow)";
            ...
            foreach (var store in stores)
            {
                ...
                logger.Info("Inserted PS_BU {0}", store.PS_BU);
            }
        ...
        {
            var rightNow = DateTime.Now;
            var lastupdatedby = "StoreSync";
            var statement = @"UPDATE STORE 
                                Set STATUS_ID = @STATUS_ID, 
                                    LAST_UPDATED_BY = @LASTUPDATEDBY, 
                                    LAST_UPDATED_DATE = @rightNow where ID = @ID";
            ...
            foreach (var store in stores)
            {
                try
                {
                    ...
                    logger.Info("Updated PS_BU {0}", store.PS_BU);
                }
                catch (Exception ex)
                {

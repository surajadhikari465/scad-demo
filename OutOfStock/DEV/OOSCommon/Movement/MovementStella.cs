...
namespace OOSCommon.Movement
{
    ...
    public class StellaMovementDataSupplimental
    {
        ...
        public DateTime? LastDateOfSales { get; set; }
        public string DaysOfMovement { get; set; }
    ...
    public class MovementStella : IMovement
    {
        ...
        public List<StellaMovementDataSupplimental> GetLastMovementDates(List<string> upcs, string storePS_BU, DateTime oosDate)
        {
            ...
            var translatedUpcs = MovementStellaUpcSpecification.TranslateUpc(upcs);
                var movementDataTemplate = string.Format(@"SELECT 
                    ...
                    WHERE   STORE='{0}' AND 
                            UPC in ({1}) AND
                            MVMNT_DATE BETWEEN to_date('{2:MM/dd/yyyy}', 'MM/DD/YYYY') - {3} AND to_date('{2:MM/dd/yyyy}', 'MM/DD/YYYY')
                            GROUP BY UPC, STORE",
                                storePS_BU,
                                "{0}", oosDate,
            ...
            {
                return string.Format(movementDataTemplate, segment);
                                                           ...
                                                           DateTime enddate)
        {
            ...
            var actualSalesTemplate = string.Format(@" select
                                             ...
                                             from       STELLA.MVMNT_DAILY 
                                             where      mvmnt_date between to_date('{2:MM/dd/yyyy}', 'MM/DD/YYYY') and to_date('{3:MM/dd/yyyy}', 'MM/DD/YYYY') and
                                                        store in ('{0}') and
                                                        upc in ({1}) ",
                                                                            "{0}",
                                                                            "{1}", begindate, enddate);
            ...
            {
                query.AddRange(PS_BUs.Select(psBu =>  string.Format(actualSalesTemplate, psBu, segment)));
            }
        ...
        private string MovementSelectLinkedSQL(string sql, string upcSet)
        {
            ...
            return string.Format(sql, upcSet); 
        ...
        private IEnumerable<MovementStella> ExecuteForMovement(string sqlQuery)
        {
            ...
            using (var oracle = new OracleDataAccess(AppConfig.ConnectionStrings["STELLA"].ConnectionString, 600))
            {
                return oracle.ReturnList<MovementStella>(CommandType.Text, sqlQuery);
            }
        ...
        private IEnumerable<StellaMovementDataActualSales> ExecuteForActualSales(string sqlQuery)
        {
            ...
            using (var oracle = new OracleDataAccess(AppConfig.ConnectionStrings["STELLA"].ConnectionString, 300))
            {
                return oracle.ReturnList<StellaMovementDataActualSales>(CommandType.Text, sqlQuery);
            }

            //using (var oosDataContext = new System.Data.Linq.DataContext(vimConnectionString))
            //{
            //    oosDataContext.CommandTimeout = 600;
        ...
        private IEnumerable<StellaMovementDataSupplimental> ExecuteForOtherStellaData(string sqlQuery)
        {
            using (var oracle = new OracleDataAccess(AppConfig.ConnectionStrings["STELLA"].ConnectionString, 300))
            {
                var items = oracle.ReturnList<StellaMovementDataSupplimental>(CommandType.Text, sqlQuery);
                return items;
            }

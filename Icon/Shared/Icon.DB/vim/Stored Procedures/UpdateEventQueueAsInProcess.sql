CREATE PROCEDURE [vim].[UpdateEventQueueAsInProcess]
	@Instance int,
	@NumberOfRows int,
...
BEGIN
	...
	FROM vim.EventQueue eq WITH (ROWLOCK, UPDLOCK, READPAST)
	...
	(
		SELECT TOP(@NumberOfRows)
			eq.InProcessBy
		FROM 
			vim.EventQueue	eq WITH (ROWLOCK, UPDLOCK, READPAST) -- force row locking for concurrency purposes

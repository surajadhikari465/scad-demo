...
AS (
	SELECT mpn.itemid,
		mpn.traitValue
	FROM itemtrait mpn
	JOIN trait t ON mpn.traitid = t.traitid
		AND t.traitCode = 'mpn'
	),
prd
AS (
...
SELECT 
	sc.itemid AS [ItemID],
	--Wrap Name in quotes because it can contain commas which will break the comma delimited extracts
	CASE
		WHEN ISNULL(mpn.traitValue, '') = '' THEN '"' + LTRIM(RTRIM(prd.traitValue)) + '"'
		ELSE '"' + LTRIM(RTRIM(prd.traitValue)) + '"'
	END AS [ItemName], 
	CASE 
...
LEFT JOIN mpn ON sc.itemID = mpn.itemID

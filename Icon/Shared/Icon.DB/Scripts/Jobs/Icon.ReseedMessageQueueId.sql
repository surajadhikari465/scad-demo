...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Icon.ReseedMessageQueueId', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Reseeds the app.MessageQueueItemLocale and app.MessageQueuePrice tables twice a year to prevent reaching the max value for the INT data type.', 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Reseed Message Queue Tables', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'DBCC CHECKIDENT (''app.MessageQueueItemLocale'', RESEED, 1)
GO
DBCC CHECKIDENT (''app.MessageQueuePrice'', RESEED, 1)
GO', 
		@database_name=N'Icon', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
@body= ''Icon.ReseedMessageQueueId has failed. Please check job history for details'', 
@subject = ''Icon.ReseedMessageQueueId - Failed'',

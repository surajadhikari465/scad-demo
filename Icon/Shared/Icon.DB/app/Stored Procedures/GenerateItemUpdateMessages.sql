...
BEGIN
	...
	select
		...
		ga.hierarchyClassName				AS [GlutenFreeAgency],
		he.Description						AS [HealthyEatingRating],
		ka.hierarchyClassName				AS [KosherAgency], 
		ia.[Msc]							As [Msc],
		na.hierarchyClassName				AS [NonGmoAgency],
		oa.hierarchyClassName				AS [OrganicAgency],
		ia.[PremiumBodyCare]				AS [PremiumBodyCare],
		sff.Description						AS [SeafoodFreshOrFrozen],
		sfc.Description						AS [SeafoodCatchType],
		va.hierarchyClassName				AS [VeganAgency],
	...
	from 
		...
		LEFT JOIN HierarchyClass		ga			ON	ia.GlutenFreeAgencyID		= ga.hierarchyClassID
														
		LEFT JOIN HierarchyClass		ka			ON	ia.KosherAgencyID			= ka.hierarchyClassID
														
		LEFT JOIN HierarchyClass		na			ON	ia.NonGmoAgencyID			= na.hierarchyClassID
														
		LEFT JOIN HierarchyClass		oa			ON	ia.OrganicAgencyID			= oa.hierarchyClassID
														
		LEFT JOIN HierarchyClass		va			ON	ia.VeganAgencyID			= va.hierarchyClassID

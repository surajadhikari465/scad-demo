namespace Icon.Monitoring.Monitors
{
    ...
    public class MammothActivePriceServiceMonitor : TimedControllerMonitor
    {
        private const string JobName = "Active Price Service";
        private const string JobScheduleStatusReady = "ready";

        private readonly IMammothActivePriceServiceMonitorSettings activePriceSettings;
        ...
        public MammothActivePriceServiceMonitor(
            IMonitorSettings settings,
            IMammothActivePriceServiceMonitorSettings activePriceSettings,
            ...
            ILogger logger)
        {
            ...
            this.activePriceSettings = activePriceSettings;
        ...
        {
            activePriceSettings.Load();

            foreach (var regionEnabledKvp in activePriceSettings.ActivePriceServiceMonitorEnabledByRegion)
            {
                if (regionEnabledKvp.Value)
                {
                    DateTime expectedUtcCompletionTime = activePriceSettings.ActivePriceServiceCompletionUtcTimeByRegion[regionEnabledKvp.Key];
        ...
        private void VerifyControllerCompletedOnTime(string region, DateTime expectedUtcCompletionTime)
        {
            try
            {
                ...
                if (jobSchedule != null && jobSchedule.Enabled)
                {
                    ...
                    {
                        TriggerOpsgenieAlertIfNoCachedOpsgenieExists(
                            region,
        ...
        private void TriggerOpsgenieAlertIfNoCachedOpsgenieExists(string region, string errorMessage, Dictionary<string, string> jsonDetails)
        {
            ...
            else
            {
                ...
                var response = this.opsgenieTrigger.TriggerAlert("Mammoth Active Price Service Issue",errorMessage, jsonDetails);

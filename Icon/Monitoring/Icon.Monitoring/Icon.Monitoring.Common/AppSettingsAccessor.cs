...
namespace Icon.Monitoring.Common
{
    public class AppSettingsAccessor
    {
        ...
        public static double GetDoubleSetting(string settingName, bool required = true)
        {
            string setting = ConfigurationManager.AppSettings[settingName];
            ValidateSetting(settingName, setting, required);

            double doubleSetting = 0;
            if (double.TryParse(setting, out doubleSetting) || !required)
            {

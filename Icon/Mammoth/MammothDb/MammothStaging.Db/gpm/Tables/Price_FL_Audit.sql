CREATE TABLE [gpm].[Price_FL_Audit](
	   ...
	   CONSTRAINT [PK_GpmPrice_FL] PRIMARY KEY NONCLUSTERED ([Region] ASC, [ItemID] ASC, [BusinessUnitID] ASC, [StartDate] ASC, [PriceType] ASC) WITH (FILLFACTOR = 100),
	   CONSTRAINT [CK_Gpm_Price_FL_Region] CHECK (Region = 'FL')
)
GO

CREATE CLUSTERED INDEX [CIX_GpmPrice_FL]
    ON [gpm].[Price_FL_Audit]([Region] ASC, [PriceID] ASC) WITH (FILLFACTOR = 100)
GO

CREATE INDEX [IX_Price_FL_StartDate] ON [gpm].[Price_FL_Audit] ([StartDate])

CREATE TABLE [dbo].[ItemAttributes_Locale_NA] (
    ...
    CONSTRAINT [PK_ItemAttributes_Locale_NA] PRIMARY KEY CLUSTERED ([Region] ASC, [ItemAttributeLocaleID] ASC) WITH (FILLFACTOR = 100) ON [FG_NA],
    CONSTRAINT [CK_ItemAttributes_Locale_NA_Restriction_Age] CHECK ([Restriction_Age]=(21) OR [Restriction_Age]=(18)),
    CONSTRAINT [CK_ItemAttributes_Locale_NA_Region] CHECK ([Region] = 'NA')
) TEXTIMAGE_ON [FG_NA];
GO

CREATE INDEX [IX_ItemAttributesLocale_NA_ItemID_BusinessUnitID_Region_ItemAttributeLocaleID] ON [dbo].[ItemAttributes_Locale_NA]
(
	[ItemID] ASC,
	[BusinessUnitID] ASC,
	[Region] ASC,
...
INCLUDE ([AddedDate], [Authorized]) ON [FG_NA];
GO

CREATE UNIQUE INDEX UNQ_ItemAttributesLocale_NA_ItemID_BU on dbo.ItemAttributes_Locale_NA(ItemID, BusinessUnitID) ON [FG_NA];
GO
CREATE TABLE [dbo].[ItemAttributes_Locale_TS](
    ...
    CONSTRAINT [PK_ItemAttributes_Locale_TS] PRIMARY KEY CLUSTERED ([Region] ASC, [ItemAttributeLocaleID] ASC) WITH (FILLFACTOR = 100) ON [FG_RM],
    CONSTRAINT [CK_ItemAttributes_Locale_TS_Restriction_Age] CHECK ([Restriction_Age]=(21) OR [Restriction_Age]=(18)),
    CONSTRAINT [CK_ItemAttributes_Locale_TS_Region] CHECK ([Region] = 'TS')
) TEXTIMAGE_ON [FG_RM];
GO
...
INCLUDE ([AddedDate], [Authorized]) ON [FG_RM];
GO

CREATE UNIQUE INDEX UNQ_ItemAttributesLocale_TS_ItemID_BU on dbo.ItemAttributes_Locale_TS(ItemID, BusinessUnitID) ON [FG_RM];
GO
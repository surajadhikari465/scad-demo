CREATE TABLE [dbo].[ItemAttributes_Locale_FL] (
    ...
    CONSTRAINT [PK_ItemAttributes_Locale_FL] PRIMARY KEY CLUSTERED ([Region] ASC, [ItemAttributeLocaleID] ASC) WITH (FILLFACTOR = 100) ON [FG_FL],
    CONSTRAINT [CK_ItemAttributes_Locale_FL_Restriction_Age] CHECK ([Restriction_Age]=(21) OR [Restriction_Age]=(18)),
    CONSTRAINT [CK_ItemAttributes_Locale_FL_Region] CHECK ([Region] = 'FL')
) TEXTIMAGE_ON [FG_FL];
GO

CREATE INDEX [IX_ItemAttributesLocale_FL_ItemID_BusinessUnitID_Region_ItemAttributeLocaleID] ON [dbo].[ItemAttributes_Locale_FL]
(
	[ItemID] ASC,
	[BusinessUnitID] ASC,
	[Region] ASC,
...
INCLUDE ([AddedDate], [Authorized]) ON [FG_FL];
GO

CREATE UNIQUE INDEX UNQ_ItemAttributesLocale_FL_ItemID_BU on dbo.ItemAttributes_Locale_FL(ItemID, BusinessUnitID) ON [FG_FL];
GO
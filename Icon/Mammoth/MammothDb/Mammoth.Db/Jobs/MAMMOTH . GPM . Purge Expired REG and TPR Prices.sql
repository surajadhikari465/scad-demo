...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'MAMMOTH . GPM . Delete Expired REG and TPR Prices', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Deletes prices from all regional price tables in the gpm schema where the prices have expired', 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Delete prices and sales', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'EXEC [gpm].[PurgeExpiredRegAndTprPrices] @MaxDeleteCount = 100000, @BatchSize = 20000;', 
...
@body= ''MAMMOTH . GPM . Purge Expired REG and TPR Prices has failed. Please check job history for details'', 
@subject = ''MAMMOTH . GPM . Purge Expired REG and TPR Prices - Failed'',

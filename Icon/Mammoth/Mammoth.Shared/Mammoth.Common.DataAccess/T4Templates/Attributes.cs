
using System;
using System.CodeDom.Compiler;
using System.Collections.Generic;

namespace Mammoth.Common.DataAccess
{
    /// <summary>
    /// dbo.Attributes auto generated Ids
    /// </summary>

    [GeneratedCode("TextTemplatingFileGenerator", "10")]
    public static class Attributes
    {
		public const int AgeRestrict = 1;
		public const int AuthorizedForSale = 2;
		public const int CaseDiscountEligible = 3;
		public const int ChicagoBaby = 4;
		public const int ColorAdded = 5;
		public const int CountryOfProcessing = 6;
		public const int Discontinued = 7;
		public const int ElectronicShelfTag = 8;
		public const int Exclusive = 9;
		public const int LabelTypeDesc = 10;
		public const int LinkedScanCode = 11;
		public const int LocalItem = 12;
		public const int Locality = 13;
		public const int Msrp = 14;
		public const int NumberOfDigitsSentToScale = 15;
		public const int Origin = 16;
		public const int ProductCode = 17;
		public const int RestrictedHours = 18;
		public const int ScaleExtraText = 19;
		public const int SignCaption = 20;
		public const int SoldByWeight = 21;
		public const int TagUom = 22;
		public const int TmDiscountEligible = 23;
		public const int SignRomanceShort = 24;
		public const int SignRomanceLong = 25;
		public const int RetailUnit = 26;
		public const int CustomerFriendlyDescription = 27;
		public const int FairTradeCertified = 28;
		public const int FlexibleText = 29;
		public const int MadeWithOrganicGrapes = 30;
		public const int MadeWithBiodynamicGrapes = 31;
		public const int NutritionRequired = 32;
		public const int PrimeBeef = 33;
		public const int RainforestAlliance = 34;
		public const int Refrigerated = 35;
		public const int SmithsonianBirdFriendly = 36;
		public const int Wic = 37;
		public const int GlobalPricingProgram = 38;
		public const int OrderedByInfor = 39;
		public const int ForceTare = 40;
		public const int ShelfLife = 41;
		public const int UnwrappedTareWeight = 42;
		public const int WrappedTareWeight = 43;
		public const int UseByEab = 44;
		public const int CfsSendToScale = 45;
		public const int VendorCaseSize = 46;
		public const int VendorName = 47;
		public const int VendorItemId = 48;
		public const int IrmaVendorKey = 49;
		public const int AltRetailSize = 51;
		public const int AltRetailUom = 52;
		public const int CurrencyCode = 53;
		public const int IrmaItemKey = 54;
		public const int DefaultIdentifier = 55;
		public const int FatFreeClaim = 56;
		public const int Tsf365Eligible = 57;
		public const int Abf = 58;
		public const int Accessory = 59;
		public const int AgeGender = 60;
		public const int AirChilled = 61;
		public const int AlcoholByVolume = 62;
		public const int Allocated = 63;
		public const int AnimalWelfareRating = 64;
		public const int Appellation = 65;
		public const int BeerStyle = 66;
		public const int Biodynamic = 67;
		public const int BlackhawkCommissionDollar = 68;
		public const int BlackhawkCommissionPercent = 69;
		public const int CaseinFree = 70;
		public const int CategoryManagement = 71;
		public const int CheeseMilkType = 72;
		public const int CountryOfOrigin = 73;
		public const int Cube = 74;
		public const int DataSource = 75;
		public const int DeliverySystem = 76;
		public const int DimensionsDataSource = 77;
		public const int Disposable = 78;
		public const int DrainedWeight = 79;
		public const int DrainedWeightUom = 80;
		public const int DryAged = 81;
		public const int EcoScaleRating = 82;
		public const int EstoreEligible = 83;
		public const int EstoreNutritionRequired = 84;
		public const int ExclusiveDate = 85;
		public const int ExclusiveYesNo = 86;
		public const int FairTradeClaim = 87;
		public const int FoodStampEligible = 88;
		public const int FragranceFree = 89;
		public const int FreeRange = 90;
		public const int FreshOrFrozen = 91;
		public const int GlutenFree = 92;
		public const int GlutenFreeClaim = 93;
		public const int NonGmoTransparency = 94;
		public const int GoodBetterBest = 95;
		public const int GrassFed = 96;
		public const int Halal = 97;
		public const int Hemp = 98;
		public const int Homeopathic = 99;
		public const int HormoneFreeClaim = 100;
		public const int HospitalityItem = 101;
		public const int ImageMap = 102;
		public const int ItemDepth = 103;
		public const int ItemHeight = 104;
		public const int PackageUnit = 105;
		public const int ItemWeight = 106;
		public const int ItemWidth = 107;
		public const int IxOneBrand = 108;
		public const int IxOneId = 109;
		public const int JuiceContent = 110;
		public const int KitchenDescription = 111;
		public const int KitchenItem = 112;
		public const int Kosher = 113;
		public const int KosherClaim = 114;
		public const int Labeling = 115;
		public const int Line = 116;
		public const int LineExtension = 117;
		public const int LocalLoanProducer = 118;
		public const int LowFatClaim = 119;
		public const int MadeInHouse = 120;
		public const int ManufacturerItemNumber = 121;
		public const int Msc = 122;
		public const int Multipurpose = 123;
		public const int NaturalClaim = 124;
		public const int NoSulfites = 125;
		public const int NonGmoClaim = 126;
		public const int NonGmo = 127;
		public const int Notes = 128;
		public const int Organic = 129;
		public const int OrganicClaim = 130;
		public const int OrganicPersonalCare = 131;
		public const int Other3pEligible = 132;
		public const int PackageGroup = 133;
		public const int PackageGroupType = 134;
		public const int PackagingType = 135;
		public const int Paleo = 136;
		public const int PaleoClaim = 137;
		public const int PastureRaised = 138;
		public const int PmdVerified = 139;
		public const int PosDescription = 140;
		public const int PosScaleTare = 141;
		public const int PremiumBodyCare = 142;
		public const int PriceLine = 143;
		public const int PriceLineDescription = 144;
		public const int PrimeNowEligible = 145;
		public const int PrivateLabel = 146;
		public const int ProductDescription = 147;
		public const int ProductFlavorType = 148;
		public const int ProhibitDiscount = 149;
		public const int CheeseRaw = 150;
		public const int RegionalLocalItem = 151;
		public const int Rennet = 152;
		public const int RetailSize = 153;
		public const int SelfCheckoutItemTareGroup = 154;
		public const int SeafoodCatchType = 155;
		public const int SeasonalInAndOutGifting = 156;
		public const int ShelfLife = 157;
		public const int SkinType = 158;
		public const int Sku = 159;
		public const int Smoked = 160;
		public const int SoldHotOrCold = 161;
		public const int TravelSizeSingleUseKit = 162;
		public const int TrayDepth = 163;
		public const int TrayHeight = 164;
		public const int TrayWidth = 165;
		public const int RetailUom = 166;
		public const int Url1 = 167;
		public const int ValueAdded = 168;
		public const int VariantSize = 169;
		public const int Varietal = 170;
		public const int Vegan = 171;
		public const int VeganClaim = 172;
		public const int Vegetarian = 173;
		public const int VegetarianClaim = 174;
		public const int WfmEligible = 175;
		public const int WholeTrade = 176;
		public const int HiddenItem = 177;
		public const int CreatedBy = 178;
		public const int InsertDate = 179;
		public const int ModifiedUser = 180;
		public const int ModifiedDate = 181;
		public const int DepartmentSale = 182;
		public const int RequestNumber = 183;
		public const int Hsh = 184;
		public const int RecipeName = 185;
		public const int Allergens = 186;
		public const int Ingredients = 187;
		public const int Selenium = 188;
		public const int PolyunsaturatedFat = 189;
		public const int MonounsaturatedFat = 190;
		public const int PotassiumWeight = 191;
		public const int PotassiumPercent = 192;
		public const int DietaryFiberPercent = 193;
		public const int SolubleFiber = 194;
		public const int InsolubleFiber = 195;
		public const int SugarAlcohol = 196;
		public const int OtherCarbohydrates = 197;
		public const int ProteinPercent = 198;
		public const int Betacarotene = 199;
		public const int VitaminD = 200;
		public const int VitaminE = 201;
		public const int Thiamin = 202;
		public const int Riboflavin = 203;
		public const int Niacin = 204;
		public const int VitaminB6 = 205;
		public const int Folate = 206;
		public const int VitaminB12 = 207;
		public const int Biotin = 208;
		public const int PantothenicAcid = 209;
		public const int Phosphorous = 210;
		public const int Iodine = 211;
		public const int Magnesium = 212;
		public const int Zinc = 213;
		public const int Copper = 214;
		public const int Transfat = 215;
		public const int Om6Fatty = 216;
		public const int Om3Fatty = 217;
		public const int Starch = 218;
		public const int Chloride = 219;
		public const int Chromium = 220;
		public const int VitaminK = 221;
		public const int Manganese = 222;
		public const int Molybdenum = 223;
		public const int CaloriesFromTransFat = 224;
		public const int CaloriesSaturatedFat = 225;
		public const int ServingPerContainer = 226;
		public const int ServingSizeDesc = 227;
		public const int ServingsPerPortion = 228;
		public const int ServingUnits = 229;
		public const int SizeWeight = 230;
		public const int TransfatWeight = 231;
		public const int Rotating = 232;
		public const int PluReissueDate = 233;
		public const int NormalizedHeightDimension = 234;
		public const int NormalizedDepthDimension = 235;
		public const int NormalizedWidthDimension = 236;
		public const int NormalizedDimensionDataSource = 237;
		public const int PegHole = 238;
		public const int RequiresCooking = 239;
		public const int ThawAndServe = 240;
		public const int CuoMadeWithoutDairyIngred = 241;
		public const int RawLabel = 242;
		public const int CuoRetailLocation = 243;
		public const int 4CuoKetoFriendly = 244;
		public const int CuoPaleoFriendly = 245;
		public const int 8CuoPlantBased = 246;
		public const int MayContainPitsOrPitFragments = 247;
		public const int SeafoodColorAdded = 248;
		public const int CuoMadeWithoutGlutenIngred = 249;
		public const int CaliforniaProp65 = 250;
		public const int MerchandisableTray = 251;
		public const int ScaleItem = 252;
		public const int RichDataStatus = 253;
		public const int DetailedItemType = 254;
		public const int ComplianceStatus = 255;
		public const int SourcedForGood = 256;
		public const int NormalizedTrayDepthDimension = 257;
		public const int NormalizedTrayHeightDimension = 258;
		public const int NormalizedTrayWidthDimension = 259;
		public const int ProfitCenter = 260;
		public const int CanadaAllergens = 261;
		public const int CanadaIngredients = 262;
		public const int CanadaSugarPercentage = 263;
		public const int CanadaServingSizeDesc = 264;

		public class Descriptions
		{
			public const string AgeRestrict = "Age Restrict";
			public const string AuthorizedForSale = "Authorized For Sale";
			public const string CaseDiscountEligible = "Case Discount Eligible";
			public const string ChicagoBaby = "Chicago Baby";
			public const string ColorAdded = "Color Added";
			public const string CountryOfProcessing = "Country of Processing";
			public const string Discontinued = "Discontinued";
			public const string ElectronicShelfTag = "Electronic Shelf Tag";
			public const string Exclusive = "Exclusive";
			public const string LabelTypeDesc = "Label Type Desc";
			public const string LinkedScanCode = "Linked Scan Code";
			public const string LocalItem = "Local Item";
			public const string Locality = "Locality";
			public const string Msrp = "MSRP";
			public const string NumberOfDigitsSentToScale = "Number of Digits Sent To Scale";
			public const string Origin = "Origin";
			public const string ProductCode = "Product Code";
			public const string RestrictedHours = "Restricted Hours";
			public const string ScaleExtraText = "Scale Extra Text";
			public const string SignCaption = "Sign Caption";
			public const string SoldByWeight = "Sold by Weight";
			public const string TagUom = "Tag UOM";
			public const string TmDiscountEligible = "TM Discount Eligible";
			public const string SignRomanceShort = "Sign Romance Short";
			public const string SignRomanceLong = "Sign Romance Long";
			public const string RetailUnit = "Retail Unit";
			public const string CustomerFriendlyDescription = "Customer Friendly Description";
			public const string FairTradeCertified = "Fair Trade Certified";
			public const string FlexibleText = "Flexible Text";
			public const string MadeWithOrganicGrapes = "Made with Organic Grapes";
			public const string MadeWithBiodynamicGrapes = "Made with Biodynamic Grapes";
			public const string NutritionRequired = "Nutrition Required";
			public const string PrimeBeef = "Prime Beef";
			public const string RainforestAlliance = "Rainforest Alliance";
			public const string Refrigerated = "Refrigerated";
			public const string SmithsonianBirdFriendly = "Smithsonian Bird Friendly";
			public const string Wic = "WIC";
			public const string GlobalPricingProgram = "Global Pricing Program";
			public const string OrderedByInfor = "Ordered By Infor";
			public const string ForceTare = "Force Tare";
			public const string ShelfLife = "Shelf Life";
			public const string UnwrappedTareWeight = "Unwrapped Tare Weight";
			public const string WrappedTareWeight = "Wrapped Tare Weight";
			public const string UseByEab = "Use By EAB";
			public const string CfsSendToScale = "CFS Send to Scale";
			public const string VendorCaseSize = "Vendor Case Size";
			public const string VendorName = "Vendor Name";
			public const string VendorItemId = "Vendor Item ID";
			public const string IrmaVendorKey = "IRMA Vendor Key";
			public const string AltRetailSize = "Alt Retail Size";
			public const string AltRetailUom = "Alt Retail Uom";
			public const string CurrencyCode = "Currency Code";
			public const string IrmaItemKey = "IRMA Item Key";
			public const string DefaultIdentifier = "Default Identifier";
			public const string FatFreeClaim = "Fat Free Claim";
			public const string Tsf365Eligible = "TSF (365) Eligible";
			public const string Abf = "ABF";
			public const string Accessory = "Accessory";
			public const string AgeGender = "Age/Gender";
			public const string AirChilled = "Air Chilled";
			public const string AlcoholByVolume = "Alcohol By Volume";
			public const string Allocated = "Allocated";
			public const string AnimalWelfareRating = "Animal Welfare Rating";
			public const string Appellation = "Appellation";
			public const string BeerStyle = "Beer Style";
			public const string Biodynamic = "Biodynamic";
			public const string BlackhawkCommissionDollar = "Blackhawk Commission Dollar";
			public const string BlackhawkCommissionPercent = "Blackhawk Commission Percent";
			public const string CaseinFree = "Casein Free";
			public const string CategoryManagement = "Category Management";
			public const string CheeseMilkType = "Cheese Milk Type";
			public const string CountryOfOrigin = "Country of Origin";
			public const string Cube = "Cube";
			public const string DataSource = "Data Source";
			public const string DeliverySystem = "Delivery System";
			public const string DimensionsDataSource = "Dimensions Data Source";
			public const string Disposable = "Disposable";
			public const string DrainedWeight = "Drained Weight";
			public const string DrainedWeightUom = "Drained Weight UOM";
			public const string DryAged = "Dry Aged";
			public const string EcoScaleRating = "ECO Scale Rating";
			public const string EstoreEligible = "Estore Eligible";
			public const string EstoreNutritionRequired = "EStore Nutrition Required";
			public const string ExclusiveDate = "Exclusive (Date)";
			public const string ExclusiveYesNo = "Exclusive (Yes/No)";
			public const string FairTradeClaim = "Fair Trade Claim";
			public const string FoodStampEligible = "Food Stamp Eligible";
			public const string FragranceFree = "Fragrance Free";
			public const string FreeRange = "Free Range";
			public const string FreshOrFrozen = "Fresh or Frozen";
			public const string GlutenFree = "Gluten Free";
			public const string GlutenFreeClaim = "Gluten Free Claim";
			public const string NonGmoTransparency = "NON-GMO Transparency";
			public const string GoodBetterBest = "Good Better Best";
			public const string GrassFed = "Grass Fed";
			public const string Halal = "Halal";
			public const string Hemp = "Hemp";
			public const string Homeopathic = "Homeopathic";
			public const string HormoneFreeClaim = "Hormone Free Claim";
			public const string HospitalityItem = "Hospitality Item";
			public const string ImageMap = "Image Map";
			public const string ItemDepth = "Item Depth";
			public const string ItemHeight = "Item Height";
			public const string PackageUnit = "Package Unit";
			public const string ItemWeight = "Item Weight";
			public const string ItemWidth = "Item Width";
			public const string IxOneBrand = "IX One Brand";
			public const string IxOneId = "IX One ID";
			public const string JuiceContent = "Juice Content";
			public const string KitchenDescription = "Kitchen Description";
			public const string KitchenItem = "Kitchen Item";
			public const string Kosher = "Kosher";
			public const string KosherClaim = "Kosher Claim";
			public const string Labeling = "Labeling";
			public const string Line = "Line";
			public const string LineExtension = "Line Extension";
			public const string LocalLoanProducer = "Local Loan Producer";
			public const string LowFatClaim = "Low Fat Claim";
			public const string MadeInHouse = "Made in House";
			public const string ManufacturerItemNumber = "Manufacturer Item Number";
			public const string Msc = "MSC";
			public const string Multipurpose = "Multipurpose";
			public const string NaturalClaim = "Natural Claim";
			public const string NoSulfites = "No Sulfites";
			public const string NonGmoClaim = "Non-GMO Claim";
			public const string NonGmo = "Non GMO";
			public const string Notes = "Notes";
			public const string Organic = "Organic";
			public const string OrganicClaim = "Organic Claim";
			public const string OrganicPersonalCare = "Organic Personal Care";
			public const string Other3pEligible = "Other 3P Eligible";
			public const string PackageGroup = "Package Group";
			public const string PackageGroupType = "Package Group Type";
			public const string PackagingType = "Packaging Type";
			public const string Paleo = "Paleo";
			public const string PaleoClaim = "Paleo Claim";
			public const string PastureRaised = "Pasture Raised";
			public const string PmdVerified = "PMD Verified";
			public const string PosDescription = "POS Description";
			public const string PosScaleTare = "POS Scale Tare";
			public const string PremiumBodyCare = "Premium Body Care";
			public const string PriceLine = "Price Line";
			public const string PriceLineDescription = "Price Line Description";
			public const string PrimeNowEligible = "Prime Now Eligible";
			public const string PrivateLabel = "Private Label";
			public const string ProductDescription = "Product Description";
			public const string ProductFlavorType = "Product Flavor/Type";
			public const string ProhibitDiscount = "Prohibit Discount";
			public const string CheeseRaw = "Cheese Raw";
			public const string RegionalLocalItem = "Regional Local Item";
			public const string Rennet = "Rennet";
			public const string RetailSize = "Retail Size";
			public const string SelfCheckoutItemTareGroup = "Self Checkout Item Tare Group";
			public const string SeafoodCatchType = "Seafood Catch Type";
			public const string SeasonalInAndOutGifting = "Seasonal In and Out/Gifting";
			public const string ShelfLife = "Shelf Life";
			public const string SkinType = "Skin Type";
			public const string Sku = "SKU";
			public const string Smoked = "Smoked";
			public const string SoldHotOrCold = "Sold Hot or Cold";
			public const string TravelSizeSingleUseKit = "Travel Size/Single Use/Kit";
			public const string TrayDepth = "Tray Depth";
			public const string TrayHeight = "Tray Height";
			public const string TrayWidth = "Tray Width";
			public const string RetailUom = "Retail UOM";
			public const string Url1 = "URL1";
			public const string ValueAdded = "Value Added";
			public const string VariantSize = "Variant Size";
			public const string Varietal = "Varietal";
			public const string Vegan = "Vegan";
			public const string VeganClaim = "Vegan Claim";
			public const string Vegetarian = "Vegetarian";
			public const string VegetarianClaim = "Vegetarian Claim";
			public const string WfmEligible = "WFM Eligible";
			public const string WholeTrade = "Whole Trade";
			public const string HiddenItem = "Hidden Item";
			public const string CreatedBy = "Created By";
			public const string InsertDate = "Insert Date";
			public const string ModifiedUser = "Modified User";
			public const string ModifiedDate = "Modified Date";
			public const string DepartmentSale = "Department Sale";
			public const string RequestNumber = "Request Number";
			public const string Hsh = "HSH";
			public const string RecipeName = "Recipe Name";
			public const string Allergens = "Allergens";
			public const string Ingredients = "Ingredients";
			public const string Selenium = "Selenium";
			public const string PolyunsaturatedFat = "Polyunsaturated Fat";
			public const string MonounsaturatedFat = "Monounsaturated Fat";
			public const string PotassiumWeight = "Potassium Weight";
			public const string PotassiumPercent = "Potassium Percent";
			public const string DietaryFiberPercent = "Dietary Fiber Percent";
			public const string SolubleFiber = "Soluble Fiber";
			public const string InsolubleFiber = "Insoluble Fiber";
			public const string SugarAlcohol = "Sugar Alcohol";
			public const string OtherCarbohydrates = "Other Carbohydrates";
			public const string ProteinPercent = "Protein Percent";
			public const string Betacarotene = "Betacarotene";
			public const string VitaminD = "Vitamin D";
			public const string VitaminE = "Vitamin E";
			public const string Thiamin = "Thiamin";
			public const string Riboflavin = "Riboflavin";
			public const string Niacin = "Niacin";
			public const string VitaminB6 = "Vitamin B6";
			public const string Folate = "Folate";
			public const string VitaminB12 = "Vitamin B12";
			public const string Biotin = "Biotin";
			public const string PantothenicAcid = "Pantothenic Acid";
			public const string Phosphorous = "Phosphorous";
			public const string Iodine = "Iodine";
			public const string Magnesium = "Magnesium";
			public const string Zinc = "Zinc";
			public const string Copper = "Copper";
			public const string Transfat = "Transfat";
			public const string Om6Fatty = "Om6 Fatty";
			public const string Om3Fatty = "Om3 Fatty";
			public const string Starch = "Starch";
			public const string Chloride = "Chloride";
			public const string Chromium = "Chromium";
			public const string VitaminK = "Vitamin K";
			public const string Manganese = "Manganese";
			public const string Molybdenum = "Molybdenum";
			public const string CaloriesFromTransFat = "Calories From Trans Fat";
			public const string CaloriesSaturatedFat = "Calories Saturated Fat";
			public const string ServingPerContainer = "Serving Per Container";
			public const string ServingSizeDesc = "Serving Size Desc";
			public const string ServingsPerPortion = "Servings Per Portion";
			public const string ServingUnits = "Serving Units";
			public const string SizeWeight = "Size Weight";
			public const string TransfatWeight = "Transfat Weight";
			public const string Rotating = "Rotating";
			public const string PluReissueDate = "PLU Reissue Date";
			public const string NormalizedHeightDimension = "Normalized Height Dimension";
			public const string NormalizedDepthDimension = "Normalized Depth Dimension";
			public const string NormalizedWidthDimension = "Normalized Width Dimension";
			public const string NormalizedDimensionDataSource = "Normalized Dimension Data Source";
			public const string PegHole = "Peg Hole";
			public const string RequiresCooking = "Requires Cooking";
			public const string ThawAndServe = "Thaw and Serve";
			public const string CuoMadeWithoutDairyIngred = "CUO Made Without Dairy Ingred";
			public const string RawLabel = "Raw Label";
			public const string CuoRetailLocation = "CUO Retail Location";
			public const string 4CuoKetoFriendly = "4.	CUO Keto Friendly";
			public const string CuoPaleoFriendly = "CUO Paleo Friendly";
			public const string 8CuoPlantBased = "8.	CUO Plant Based";
			public const string MayContainPitsOrPitFragments = "May Contain Pits Or Pit Fragments";
			public const string SeafoodColorAdded = "Seafood Color Added";
			public const string CuoMadeWithoutGlutenIngred = "CUO Made Without Gluten Ingred";
			public const string CaliforniaProp65 = "California Prop 65";
			public const string MerchandisableTray = "Merchandisable Tray";
			public const string ScaleItem = "Scale Item";
			public const string RichDataStatus = "Rich Data Status";
			public const string DetailedItemType = "Detailed Item Type";
			public const string ComplianceStatus = "Compliance Status";
			public const string SourcedForGood = "Sourced for Good";
			public const string NormalizedTrayDepthDimension = "Normalized Tray Depth Dimension";
			public const string NormalizedTrayHeightDimension = "Normalized Tray Height Dimension";
			public const string NormalizedTrayWidthDimension = "Normalized Tray Width Dimension";
			public const string ProfitCenter = "Profit Center";
			public const string CanadaAllergens = "Canada Allergens";
			public const string CanadaIngredients = "Canada Ingredients";
			public const string CanadaSugarPercentage = "Canada Sugar Percentage";
			public const string CanadaServingSizeDesc = "Canada Serving Size";

			private static Dictionary<string, string> codeToDescriptionsDictionary = new Dictionary<string, string>
			{
				{ "AGE", "Age Restrict" },
				{ "NA", "Authorized For Sale" },
				{ "CSD", "Case Discount Eligible" },
				{ "CHB", "Chicago Baby" },
				{ "CLA", "Color Added" },
				{ "COP", "Country of Processing" },
				{ "DSC", "Discontinued" },
				{ "EST", "Electronic Shelf Tag" },
				{ "EX", "Exclusive" },
				{ "LTD", "Label Type Desc" },
				{ "LSC", "Linked Scan Code" },
				{ "LI", "Local Item" },
				{ "LCY", "Locality" },
				{ "SRP", "MSRP" },
				{ "NDS", "Number of Digits Sent To Scale" },
				{ "ORN", "Origin" },
				{ "PCD", "Product Code" },
				{ "RES", "Restricted Hours" },
				{ "SET", "Scale Extra Text" },
				{ "SC", "Sign Caption" },
				{ "SBW", "Sold by Weight" },
				{ "TU", "Tag UOM" },
				{ "TMD", "TM Discount Eligible" },
				{ "SHT", "Sign Romance Short" },
				{ "LNG", "Sign Romance Long" },
				{ "RTU", "Retail Unit" },
				{ "CFD", "Customer Friendly Description" },
				{ "FTC", "Fair Trade Certified" },
				{ "FXT", "Flexible Text" },
				{ "MOG", "Made with Organic Grapes" },
				{ "MBG", "Made with Biodynamic Grapes" },
				{ "NR", "Nutrition Required" },
				{ "PRB", "Prime Beef" },
				{ "RFA", "Rainforest Alliance" },
				{ "RFD", "Refrigerated" },
				{ "SMF", "Smithsonian Bird Friendly" },
				{ "WIC", "WIC" },
				{ "GPP", "Global Pricing Program" },
				{ "OBI", "Ordered By Infor" },
				{ "FTA", "Force Tare" },
				{ "SHL", "Shelf Life" },
				{ "UTA", "Unwrapped Tare Weight" },
				{ "WTA", "Wrapped Tare Weight" },
				{ "EAB", "Use By EAB" },
				{ "CFS", "CFS Send to Scale" },
				{ "VCS", "Vendor Case Size" },
				{ "VND", "Vendor Name" },
				{ "VIN", "Vendor Item ID" },
				{ "VNK", "IRMA Vendor Key" },
				{ "ASZ", "Alt Retail Size" },
				{ "AUM", "Alt Retail Uom" },
				{ "CUR", "Currency Code" },
				{ "IIK", "IRMA Item Key" },
				{ "IDI", "Default Identifier" },
				{ "FF", "Fat Free Claim" },
				{ "TSE", "TSF (365) Eligible" },
				{ "ABF", "ABF" },
				{ "AC", "Accessory" },
				{ "AG", "Age/Gender" },
				{ "ACH", "Air Chilled" },
				{ "ABV", "Alcohol By Volume" },
				{ "AL", "Allocated" },
				{ "AWR", "Animal Welfare Rating" },
				{ "APL", "Appellation" },
				{ "BES", "Beer Style" },
				{ "BIO", "Biodynamic" },
				{ "BCD", "Blackhawk Commission Dollar" },
				{ "BCP", "Blackhawk Commission Percent" },
				{ "CF", "Casein Free" },
				{ "CM", "Category Management" },
				{ "CMT", "Cheese Milk Type" },
				{ "COO", "Country of Origin" },
				{ "CUB", "Cube" },
				{ "DAT", "Data Source" },
				{ "DS", "Delivery System" },
				{ "DDS", "Dimensions Data Source" },
				{ "DIS", "Disposable" },
				{ "DW", "Drained Weight" },
				{ "DWU", "Drained Weight UOM" },
				{ "DAG", "Dry Aged" },
				{ "ECO", "ECO Scale Rating" },
				{ "ESE", "Estore Eligible" },
				{ "ESN", "EStore Nutrition Required" },
				{ "EXD", "Exclusive (Date)" },
				{ "EXL", "Exclusive (Yes/No)" },
				{ "FT", "Fair Trade Claim" },
				{ "FSE", "Food Stamp Eligible" },
				{ "FRF", "Fragrance Free" },
				{ "FRR", "Free Range" },
				{ "SFF", "Fresh or Frozen" },
				{ "GF", "Gluten Free" },
				{ "GFC", "Gluten Free Claim" },
				{ "NGT", "NON-GMO Transparency" },
				{ "GBB", "Good Better Best" },
				{ "GRF", "Grass Fed" },
				{ "HAL", "Halal" },
				{ "HEM", "Hemp" },
				{ "HOM", "Homeopathic" },
				{ "HFC", "Hormone Free Claim" },
				{ "HI", "Hospitality Item" },
				{ "IM", "Image Map" },
				{ "IDP", "Item Depth" },
				{ "IHT", "Item Height" },
				{ "PKG", "Package Unit" },
				{ "IWT", "Item Weight" },
				{ "IWD", "Item Width" },
				{ "IXO", "IX One Brand" },
				{ "IXI", "IX One ID" },
				{ "JC", "Juice Content" },
				{ "KD", "Kitchen Description" },
				{ "KI", "Kitchen Item" },
				{ "KSH", "Kosher" },
				{ "KC", "Kosher Claim" },
				{ "LBL", "Labeling" },
				{ "LIN", "Line" },
				{ "LEX", "Line Extension" },
				{ "LLP", "Local Loan Producer" },
				{ "LF", "Low Fat Claim" },
				{ "MIH", "Made in House" },
				{ "MIN", "Manufacturer Item Number" },
				{ "MSC", "MSC" },
				{ "MP", "Multipurpose" },
				{ "NC", "Natural Claim" },
				{ "NS", "No Sulfites" },
				{ "NGC", "Non-GMO Claim" },
				{ "NGM", "Non GMO" },
				{ "NTS", "Notes" },
				{ "OG", "Organic" },
				{ "OC", "Organic Claim" },
				{ "OPC", "Organic Personal Care" },
				{ "OTE", "Other 3P Eligible" },
				{ "PG", "Package Group" },
				{ "PGT", "Package Group Type" },
				{ "PT", "Packaging Type" },
				{ "PLO", "Paleo" },
				{ "PC", "Paleo Claim" },
				{ "PAS", "Pasture Raised" },
				{ "PV", "PMD Verified" },
				{ "POS", "POS Description" },
				{ "SCT", "POS Scale Tare" },
				{ "PBC", "Premium Body Care" },
				{ "PL", "Price Line" },
				{ "PLD", "Price Line Description" },
				{ "PNE", "Prime Now Eligible" },
				{ "PRL", "Private Label" },
				{ "PRD", "Product Description" },
				{ "PFT", "Product Flavor/Type" },
				{ "PRH", "Prohibit Discount" },
				{ "CR", "Cheese Raw" },
				{ "RL", "Regional Local Item" },
				{ "RE", "Rennet" },
				{ "RSZ", "Retail Size" },
				{ "ITG", "Self Checkout Item Tare Group" },
				{ "SFT", "Seafood Catch Type" },
				{ "SIO", "Seasonal In and Out/Gifting" },
				{ "SLF", "Shelf Life" },
				{ "SKT", "Skin Type" },
				{ "SKU", "SKU" },
				{ "SMK", "Smoked" },
				{ "HC", "Sold Hot or Cold" },
				{ "TS", "Travel Size/Single Use/Kit" },
				{ "TDP", "Tray Depth" },
				{ "THT", "Tray Height" },
				{ "TWD", "Tray Width" },
				{ "RUM", "Retail UOM" },
				{ "SIU", "URL1" },
				{ "VA", "Value Added" },
				{ "VS", "Variant Size" },
				{ "VAR", "Varietal" },
				{ "VGN", "Vegan" },
				{ "VC", "Vegan Claim" },
				{ "VEG", "Vegetarian" },
				{ "VCL", "Vegetarian Claim" },
				{ "WFE", "WFM Eligible" },
				{ "WT", "Whole Trade" },
				{ "HID", "Hidden Item" },
				{ "CRB", "Created By" },
				{ "INS", "Insert Date" },
				{ "USR", "Modified User" },
				{ "MOD", "Modified Date" },
				{ "DPT", "Department Sale" },
				{ "RQN", "Request Number" },
				{ "HSH", "HSH" },
				{ "RCN", "Recipe Name" },
				{ "ALR", "Allergens" },
				{ "ING", "Ingredients" },
				{ "SLM", "Selenium" },
				{ "PUF", "Polyunsaturated Fat" },
				{ "MUF", "Monounsaturated Fat" },
				{ "PTW", "Potassium Weight" },
				{ "PTP", "Potassium Percent" },
				{ "DFP", "Dietary Fiber Percent" },
				{ "SBF", "Soluble Fiber" },
				{ "ISF", "Insoluble Fiber" },
				{ "SAL", "Sugar Alcohol" },
				{ "OCH", "Other Carbohydrates" },
				{ "PPT", "Protein Percent" },
				{ "BCN", "Betacarotene" },
				{ "VTD", "Vitamin D" },
				{ "VTE", "Vitamin E" },
				{ "THM", "Thiamin" },
				{ "RFN", "Riboflavin" },
				{ "NAC", "Niacin" },
				{ "VB6", "Vitamin B6" },
				{ "FLT", "Folate" },
				{ "B12", "Vitamin B12" },
				{ "BTN", "Biotin" },
				{ "PAD", "Pantothenic Acid" },
				{ "PPH", "Phosphorous" },
				{ "IDN", "Iodine" },
				{ "MGM", "Magnesium" },
				{ "ZNC", "Zinc" },
				{ "CPR", "Copper" },
				{ "TSF", "Transfat" },
				{ "O6F", "Om6 Fatty" },
				{ "O3F", "Om3 Fatty" },
				{ "STR", "Starch" },
				{ "CHR", "Chloride" },
				{ "CHM", "Chromium" },
				{ "VTK", "Vitamin K" },
				{ "MGE", "Manganese" },
				{ "MBD", "Molybdenum" },
				{ "CTF", "Calories From Trans Fat" },
				{ "CSF", "Calories Saturated Fat" },
				{ "SPC", "Serving Per Container" },
				{ "SSD", "Serving Size Desc" },
				{ "SPP", "Servings Per Portion" },
				{ "SUT", "Serving Units" },
				{ "SWT", "Size Weight" },
				{ "TFW", "Transfat Weight" },
				{ "ROT", "Rotating" },
				{ "PR", "PLU Reissue Date" },
				{ "NH", "Normalized Height Dimension" },
				{ "ND", "Normalized Depth Dimension" },
				{ "NW", "Normalized Width Dimension" },
				{ "DSN", "Normalized Dimension Data Source" },
				{ "PH", "Peg Hole" },
				{ "RC", "Requires Cooking" },
				{ "TAS", "Thaw and Serve" },
				{ "MWD", "CUO Made Without Dairy Ingred" },
				{ "RAW", "Raw Label" },
				{ "CRL", "CUO Retail Location" },
				{ "KF", "4.	CUO Keto Friendly" },
				{ "PF", "CUO Paleo Friendly" },
				{ "PB", "8.	CUO Plant Based" },
				{ "MCP", "May Contain Pits Or Pit Fragments" },
				{ "SCA", "Seafood Color Added" },
				{ "MWG", "CUO Made Without Gluten Ingred" },
				{ "CP", "California Prop 65" },
				{ "MT", "Merchandisable Tray" },
				{ "SCL", "Scale Item" },
				{ "RDS", "Rich Data Status" },
				{ "DIT", "Detailed Item Type" },
				{ "CST", "Compliance Status" },
				{ "SFG", "Sourced for Good" },
				{ "NTD", "Normalized Tray Depth Dimension" },
				{ "NTH", "Normalized Tray Height Dimension" },
				{ "NTW", "Normalized Tray Width Dimension" },
				{ "PFC", "Profit Center" },
				{ "CDA", "Canada Allergens" },
				{ "CDI", "Canada Ingredients" },
				{ "CDS", "Canada Sugar Percentage" }
			};
			public static Dictionary<string, string> ByCode { get { return codeToDescriptionsDictionary; } }
		}

		public class Codes
		{
			public const string AgeRestrict = "AGE";
			public const string AuthorizedForSale = "NA";
			public const string CaseDiscountEligible = "CSD";
			public const string ChicagoBaby = "CHB";
			public const string ColorAdded = "CLA";
			public const string CountryOfProcessing = "COP";
			public const string Discontinued = "DSC";
			public const string ElectronicShelfTag = "EST";
			public const string Exclusive = "EX";
			public const string LabelTypeDesc = "LTD";
			public const string LinkedScanCode = "LSC";
			public const string LocalItem = "LI";
			public const string Locality = "LCY";
			public const string Msrp = "SRP";
			public const string NumberOfDigitsSentToScale = "NDS";
			public const string Origin = "ORN";
			public const string ProductCode = "PCD";
			public const string RestrictedHours = "RES";
			public const string ScaleExtraText = "SET";
			public const string SignCaption = "SC";
			public const string SoldByWeight = "SBW";
			public const string TagUom = "TU";
			public const string TmDiscountEligible = "TMD";
			public const string SignRomanceShort = "SHT";
			public const string SignRomanceLong = "LNG";
			public const string RetailUnit = "RTU";
			public const string CustomerFriendlyDescription = "CFD";
			public const string FairTradeCertified = "FTC";
			public const string FlexibleText = "FXT";
			public const string MadeWithOrganicGrapes = "MOG";
			public const string MadeWithBiodynamicGrapes = "MBG";
			public const string NutritionRequired = "NR";
			public const string PrimeBeef = "PRB";
			public const string RainforestAlliance = "RFA";
			public const string Refrigerated = "RFD";
			public const string SmithsonianBirdFriendly = "SMF";
			public const string Wic = "WIC";
			public const string GlobalPricingProgram = "GPP";
			public const string OrderedByInfor = "OBI";
			public const string ForceTare = "FTA";
			public const string ShelfLife = "SHL";
			public const string UnwrappedTareWeight = "UTA";
			public const string WrappedTareWeight = "WTA";
			public const string UseByEab = "EAB";
			public const string CfsSendToScale = "CFS";
			public const string VendorCaseSize = "VCS";
			public const string VendorName = "VND";
			public const string VendorItemId = "VIN";
			public const string IrmaVendorKey = "VNK";
			public const string AltRetailSize = "ASZ";
			public const string AltRetailUom = "AUM";
			public const string CurrencyCode = "CUR";
			public const string IrmaItemKey = "IIK";
			public const string DefaultIdentifier = "IDI";
			public const string FatFreeClaim = "FF";
			public const string Tsf365Eligible = "TSE";
			public const string Abf = "ABF";
			public const string Accessory = "AC";
			public const string AgeGender = "AG";
			public const string AirChilled = "ACH";
			public const string AlcoholByVolume = "ABV";
			public const string Allocated = "AL";
			public const string AnimalWelfareRating = "AWR";
			public const string Appellation = "APL";
			public const string BeerStyle = "BES";
			public const string Biodynamic = "BIO";
			public const string BlackhawkCommissionDollar = "BCD";
			public const string BlackhawkCommissionPercent = "BCP";
			public const string CaseinFree = "CF";
			public const string CategoryManagement = "CM";
			public const string CheeseMilkType = "CMT";
			public const string CountryOfOrigin = "COO";
			public const string Cube = "CUB";
			public const string DataSource = "DAT";
			public const string DeliverySystem = "DS";
			public const string DimensionsDataSource = "DDS";
			public const string Disposable = "DIS";
			public const string DrainedWeight = "DW";
			public const string DrainedWeightUom = "DWU";
			public const string DryAged = "DAG";
			public const string EcoScaleRating = "ECO";
			public const string EstoreEligible = "ESE";
			public const string EstoreNutritionRequired = "ESN";
			public const string ExclusiveDate = "EXD";
			public const string ExclusiveYesNo = "EXL";
			public const string FairTradeClaim = "FT";
			public const string FoodStampEligible = "FSE";
			public const string FragranceFree = "FRF";
			public const string FreeRange = "FRR";
			public const string FreshOrFrozen = "SFF";
			public const string GlutenFree = "GF";
			public const string GlutenFreeClaim = "GFC";
			public const string NonGmoTransparency = "NGT";
			public const string GoodBetterBest = "GBB";
			public const string GrassFed = "GRF";
			public const string Halal = "HAL";
			public const string Hemp = "HEM";
			public const string Homeopathic = "HOM";
			public const string HormoneFreeClaim = "HFC";
			public const string HospitalityItem = "HI";
			public const string ImageMap = "IM";
			public const string ItemDepth = "IDP";
			public const string ItemHeight = "IHT";
			public const string PackageUnit = "PKG";
			public const string ItemWeight = "IWT";
			public const string ItemWidth = "IWD";
			public const string IxOneBrand = "IXO";
			public const string IxOneId = "IXI";
			public const string JuiceContent = "JC";
			public const string KitchenDescription = "KD";
			public const string KitchenItem = "KI";
			public const string Kosher = "KSH";
			public const string KosherClaim = "KC";
			public const string Labeling = "LBL";
			public const string Line = "LIN";
			public const string LineExtension = "LEX";
			public const string LocalLoanProducer = "LLP";
			public const string LowFatClaim = "LF";
			public const string MadeInHouse = "MIH";
			public const string ManufacturerItemNumber = "MIN";
			public const string Msc = "MSC";
			public const string Multipurpose = "MP";
			public const string NaturalClaim = "NC";
			public const string NoSulfites = "NS";
			public const string NonGmoClaim = "NGC";
			public const string NonGmo = "NGM";
			public const string Notes = "NTS";
			public const string Organic = "OG";
			public const string OrganicClaim = "OC";
			public const string OrganicPersonalCare = "OPC";
			public const string Other3pEligible = "OTE";
			public const string PackageGroup = "PG";
			public const string PackageGroupType = "PGT";
			public const string PackagingType = "PT";
			public const string Paleo = "PLO";
			public const string PaleoClaim = "PC";
			public const string PastureRaised = "PAS";
			public const string PmdVerified = "PV";
			public const string PosDescription = "POS";
			public const string PosScaleTare = "SCT";
			public const string PremiumBodyCare = "PBC";
			public const string PriceLine = "PL";
			public const string PriceLineDescription = "PLD";
			public const string PrimeNowEligible = "PNE";
			public const string PrivateLabel = "PRL";
			public const string ProductDescription = "PRD";
			public const string ProductFlavorType = "PFT";
			public const string ProhibitDiscount = "PRH";
			public const string CheeseRaw = "CR";
			public const string RegionalLocalItem = "RL";
			public const string Rennet = "RE";
			public const string RetailSize = "RSZ";
			public const string SelfCheckoutItemTareGroup = "ITG";
			public const string SeafoodCatchType = "SFT";
			public const string SeasonalInAndOutGifting = "SIO";
			public const string ShelfLife = "SLF";
			public const string SkinType = "SKT";
			public const string Sku = "SKU";
			public const string Smoked = "SMK";
			public const string SoldHotOrCold = "HC";
			public const string TravelSizeSingleUseKit = "TS";
			public const string TrayDepth = "TDP";
			public const string TrayHeight = "THT";
			public const string TrayWidth = "TWD";
			public const string RetailUom = "RUM";
			public const string Url1 = "SIU";
			public const string ValueAdded = "VA";
			public const string VariantSize = "VS";
			public const string Varietal = "VAR";
			public const string Vegan = "VGN";
			public const string VeganClaim = "VC";
			public const string Vegetarian = "VEG";
			public const string VegetarianClaim = "VCL";
			public const string WfmEligible = "WFE";
			public const string WholeTrade = "WT";
			public const string HiddenItem = "HID";
			public const string CreatedBy = "CRB";
			public const string InsertDate = "INS";
			public const string ModifiedUser = "USR";
			public const string ModifiedDate = "MOD";
			public const string DepartmentSale = "DPT";
			public const string RequestNumber = "RQN";
			public const string Hsh = "HSH";
			public const string RecipeName = "RCN";
			public const string Allergens = "ALR";
			public const string Ingredients = "ING";
			public const string Selenium = "SLM";
			public const string PolyunsaturatedFat = "PUF";
			public const string MonounsaturatedFat = "MUF";
			public const string PotassiumWeight = "PTW";
			public const string PotassiumPercent = "PTP";
			public const string DietaryFiberPercent = "DFP";
			public const string SolubleFiber = "SBF";
			public const string InsolubleFiber = "ISF";
			public const string SugarAlcohol = "SAL";
			public const string OtherCarbohydrates = "OCH";
			public const string ProteinPercent = "PPT";
			public const string Betacarotene = "BCN";
			public const string VitaminD = "VTD";
			public const string VitaminE = "VTE";
			public const string Thiamin = "THM";
			public const string Riboflavin = "RFN";
			public const string Niacin = "NAC";
			public const string VitaminB6 = "VB6";
			public const string Folate = "FLT";
			public const string VitaminB12 = "B12";
			public const string Biotin = "BTN";
			public const string PantothenicAcid = "PAD";
			public const string Phosphorous = "PPH";
			public const string Iodine = "IDN";
			public const string Magnesium = "MGM";
			public const string Zinc = "ZNC";
			public const string Copper = "CPR";
			public const string Transfat = "TSF";
			public const string Om6Fatty = "O6F";
			public const string Om3Fatty = "O3F";
			public const string Starch = "STR";
			public const string Chloride = "CHR";
			public const string Chromium = "CHM";
			public const string VitaminK = "VTK";
			public const string Manganese = "MGE";
			public const string Molybdenum = "MBD";
			public const string CaloriesFromTransFat = "CTF";
			public const string CaloriesSaturatedFat = "CSF";
			public const string ServingPerContainer = "SPC";
			public const string ServingSizeDesc = "SSD";
			public const string ServingsPerPortion = "SPP";
			public const string ServingUnits = "SUT";
			public const string SizeWeight = "SWT";
			public const string TransfatWeight = "TFW";
			public const string Rotating = "ROT";
			public const string PluReissueDate = "PR";
			public const string NormalizedHeightDimension = "NH";
			public const string NormalizedDepthDimension = "ND";
			public const string NormalizedWidthDimension = "NW";
			public const string NormalizedDimensionDataSource = "DSN";
			public const string PegHole = "PH";
			public const string RequiresCooking = "RC";
			public const string ThawAndServe = "TAS";
			public const string CuoMadeWithoutDairyIngred = "MWD";
			public const string RawLabel = "RAW";
			public const string CuoRetailLocation = "CRL";
			public const string 4CuoKetoFriendly = "KF";
			public const string CuoPaleoFriendly = "PF";
			public const string 8CuoPlantBased = "PB";
			public const string MayContainPitsOrPitFragments = "MCP";
			public const string SeafoodColorAdded = "SCA";
			public const string CuoMadeWithoutGlutenIngred = "MWG";
			public const string CaliforniaProp65 = "CP";
			public const string MerchandisableTray = "MT";
			public const string ScaleItem = "SCL";
			public const string RichDataStatus = "RDS";
			public const string DetailedItemType = "DIT";
			public const string ComplianceStatus = "CST";
			public const string SourcedForGood = "SFG";
			public const string NormalizedTrayDepthDimension = "NTD";
			public const string NormalizedTrayHeightDimension = "NTH";
			public const string NormalizedTrayWidthDimension = "NTW";
			public const string ProfitCenter = "PFC";
			public const string CanadaAllergens = "CDA";
			public const string CanadaIngredients = "CDI";
			public const string CanadaSugarPercentage = "CDS";
			public const string CanadaServingSizeDesc = "CDR";
		}
	}
}

...
{
    public class GetItemPsgDataForGpmRegionQuery : IQueryHandler<GetItemPsgDataForGpmRegionParameters, IEnumerable<ItemPsgModel>>
    {
        private IDbConnection connection;

        public GetItemPsgDataForGpmRegionQuery(IDbConnection connection)
        {
            this.connection = connection;
        }

        public IEnumerable<ItemPsgModel> Search(GetItemPsgDataForGpmRegionParameters parameters)
        {
            ...
            var psgModels = connection.Query<dynamic>($@"
                    ...
                    LEFT JOIN gpm.Price_{parameters.Region}  p ON i.ItemID = p.ItemID
            ...
            foreach (var priceGroup in nonTprs.Union(currentTprs).GroupBy(p => new { p.ItemID, p.BusinessUnitID }))
            {
                ...
                if(priceGroup.Any(p =>p.PriceType == "TPR" && !parameters.ExcludedPsNumbers.Contains(p.PSNumber)))
                {

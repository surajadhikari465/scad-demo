...
BEGIN
	...
	SELECT DISTINCT icn.*, dbo.fn_IsScaleItem(plu) as IsScaleItem INTO #distinctList	
	...
	BEGIN TRY
		...
		JOIN #distinctList		dl on ii.Identifier = dl.Plu and dl.IsScaleItem = 0
	...
	UPDATE nf
		...
		JOIN #distinctList		dl on ii.Identifier = dl.Plu  and dl.IsScaleItem = 1
	...
	JOIN ItemIdentifier		ii on	ii.Deleted_Identifier = 0
											...
											AND dl.IsScaleItem = 0
											and (not exists (select  1 from ItemNutrition inf where inf.ItemKey = ii.Item_Key)
													OR
												 not exists (select  1 from ItemNutrition inf where inf.ItemKey = ii.Item_Key and inf.NutriFactsID is null)
												)	
	...
	JOIN ItemIdentifier		ii on	ii.Deleted_Identifier = 0
											AND ii.Default_Identifier = 1
									 		AND ii.Identifier = dl.Plu
											AND dl.IsScaleItem = 1
											and (	not exists (select  1 from ItemScale its where its.Item_Key = ii.Item_Key)
													OR
													not exists (select  1 from ItemScale its where its.Item_Key = ii.Item_Key and its.Nutrifact_ID is not null)
												)


	--Insert into ItemNutrition with newly added nutrifacts
	...
	JOIN @NewNutrifacts nnf on ii.Identifier = nnf.plu and nnf.isScalePlu = 0
	...
	where nnf.isScalePlu = 1 and  not exists (select 1 from ItemScale itsl where itsl.Item_Key = ii.Item_Key)
	...
	JOIN @NewNutrifacts nnf on ii.Identifier = nnf.plu and nnf.isScalePlu = 1

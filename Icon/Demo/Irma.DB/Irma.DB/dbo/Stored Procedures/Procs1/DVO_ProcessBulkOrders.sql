...
--       create index #tmpHeadUp on #tmpHeaderUpdates (DVOOrderId);
	...
	WHERE oh.OrderHeader_ID NOT IN ( SELECT oei.OrderHeader_Id FROM ExternalOrderInformation oei WHERE oei.ExternalOrder_Id = oh.OrderExternalSourceOrderID AND oei.OrderHeader_Id = oh.orderheader_id )
	...
	FROM		OrderHeader oh
				INNER JOIN #tmpHeadersInsert th ON ( oh.DVOOrderId = th.DVOOrderId )
	WHERE oh.OrderHeader_ID NOT IN ( SELECT oei.OrderHeader_Id FROM ExternalOrderInformation oei WHERE oei.ExternalOrder_Id = @PDXSourceID AND oei.OrderHeader_Id = oh.orderheader_id )

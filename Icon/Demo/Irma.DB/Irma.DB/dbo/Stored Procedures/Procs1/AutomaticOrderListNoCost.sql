...
CREATE PROCEDURE [dbo].[AutomaticOrderListNoCost]
	@OrderHeader_ID		int,
...
AS 
	...
	-- **************************************************************************
	-- Procedure: AutomaticOrderListNoCost()
	...
	-- 02/08/11	BBB		1367	Created based upon AutomaticOrderList but with no cost calls
	-- 08/17/11	BBB		2716	Removed extraneous tables and left joins; formatted 2nd query
	-- 12/29/11 BJL		3751	Added CurrentVendorCost = ''
	-- 01/03/13 KM		9251	Check ItemOverride for new override values (Not_Available, Not_AvailableNote, Brand_Name);
	-- 01/04/13 BS		8755	Updated i.Discontinue_Item with siv.DiscontinueItem due to schema change
	-- 10/15/13	BS		1066	Create temp tables of Item_Keys and Vendor Pack information
	--							to JOIN to main query to improve performance
	--					14314	Also fixed the bug causing incorrect vendor packs.
	-- **************************************************************************
...
BEGIN      
	...
	--****************************************************************************
	--------------------------------------------------
	-- Get Vendor Pack for the latest cost record
	--------------------------------------------------
	select
		cid.StoreItemVendorID,
		cid.VendorCostHistoryID,
		cost.Package_Desc1
	into #vendorCost
	from
		(SELECT
		...
		GROUP BY
			itk.StoreItemVendorID) cid
		INNER JOIN VendorCostHistory cost (nolock) on cid.VendorCostHistoryID = cost.VendorCostHistoryID

	create nonclustered index idx_vendorCost_StoreItemVendorID on #vendorCost (StoreItemVendorID) include (VendorCostHistoryID, Package_Desc1)
	...
	BEGIN   
		...
		WHERE
			...
			AND (ik.DistSubTeam_No	= @Transfer_From_SubTeam
				OR ik.SubTeam_No	= @Transfer_From_SubTeam)
		ORDER BY
			OrderCount DESC,
			QuantityReceived DESC
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[AutomaticOrderListNoCost] TO [IRMAClientRole]
    AS [dbo];

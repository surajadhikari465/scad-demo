...
BEGIN
	...
	--Synching with JDA?  
	DECLARE @SyncJDA bit
	SELECT @SyncJDA = (SELECT FlagValue FROM dbo.InstanceDataFlags (NOLOCK) WHERE FlagKey='SyncJDA')
	...
	IF @Status = 0 -- Validated UPC & PLU flags have not been turned on for the region.
		BEGIN
			...
			Scale_Identifier FROM ItemIdentifier II (NOLOCK)
		END
	ELSE
		...
		BEGIN
			SET @SQL = @SQL + 
				' (SELECT (ISNULL(JDA_Dept, 100) - 100)
				 FROM JDA_HierarchyMapping (NOLOCK) 
				 WHERE ProdHierarchyLevel4_ID = I.ProdHierarchyLevel4_ID) AS JDA_Dept, '
		END

select @SQL = @SQL + ' 
	 (SELECT ISNULL(Value,'''') FROM KitchenRoute (NOLOCK) WHERE KitchenRoute_ID = P.KitchenRoute_ID) AS KitchenRouteValue,
...
INNER JOIN
	...
	IF @IsItemNonBatchableChanges = 1
		BEGIN
			SET @SQL = @SQL + '		
				INNER JOIN 
					dbo.ItemNonBatchableChanges inbc (nolock)
					ON I.Item_Key = inbc.Item_Key
					AND inbc.StartDate <= @Date'
		END
...
WHERE 
	...
	(icfs.SendToScale = 1 or icfs.SendToScale is null)'

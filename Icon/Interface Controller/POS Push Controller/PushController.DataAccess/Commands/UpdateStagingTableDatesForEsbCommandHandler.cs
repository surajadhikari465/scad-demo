...
{
    public class UpdateStagingTableDatesForEsbCommandHandler : ICommandHandler<UpdateStagingTableDatesForEsbCommand>
    {
        private ILogger<UpdateStagingTableDatesForEsbCommandHandler> logger;
        private IRenewableContext<IconContext> context;

        public UpdateStagingTableDatesForEsbCommandHandler(ILogger<UpdateStagingTableDatesForEsbCommandHandler> logger, IRenewableContext<IconContext> context)
        {
            this.logger = logger;
            this.context = context;
        }

        public void Execute(UpdateStagingTableDatesForEsbCommand command)
        {
            if (command.StagedPosData == null || command.StagedPosData.Count == 0)
            {
                logger.Warn("UpdateStagingTableDatesForEsbCommandHandler was called with an empty or null list.  Check the execution path in the calling method for potential errors.");
                return;
            }
            ...
            string sql = "EXEC app.UpdateStagingTableDatesForEsb @ProcessedSuccessfully, @RecordsToUpdate, @Date";
            ...
            logger.Info(String.Format("Successfully updated {0} IRMAPush record(s) with a {1} of {2} beginning with IRMAPushID {3}.",
                command.StagedPosData.Count, command.ProcessedSuccessfully ? "EsbReadyDate" : "EsbReadyFailedDate", command.Date, command.StagedPosData[0].IRMAPushID));
        }
    }
}

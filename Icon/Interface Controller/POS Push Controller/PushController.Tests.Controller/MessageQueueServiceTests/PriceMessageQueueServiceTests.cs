...
{
    [TestClass]
    public class PriceMessageQueueServiceTests
    {
        private GlobalIconContext context;
        private DbContextTransaction transaction;
        private PriceMessageQueueService priceMessageQueueService;
        private Mock<ILogger<PriceMessageQueueService>> mockLogger;
        private Mock<IEmailClient> mockEmailClient;
        private AddPriceMessagesBulkCommandHandler addMessagesBulkCommandHandler;
        private Mock<ICommandHandler<AddPriceMessagesBulkCommand>> mockAddMessagesBulkCommandHandler;
        private AddPriceMessagesRowByRowCommandHandler addMessagesRowByRowCommandHandler;
        ...
        public void Initialize()
        {
            ...
            this.mockLogger = new Mock<ILogger<PriceMessageQueueService>>();
            this.mockEmailClient = new Mock<IEmailClient>();
            this.addMessagesBulkCommandHandler = new AddPriceMessagesBulkCommandHandler(new Mock<ILogger<AddPriceMessagesBulkCommandHandler>>().Object, context);
            this.mockAddMessagesBulkCommandHandler = new Mock<ICommandHandler<AddPriceMessagesBulkCommand>>();
            this.addMessagesRowByRowCommandHandler = new AddPriceMessagesRowByRowCommandHandler(context);
        ...
        public void SaveMessagesBulk_SaveIsSuccessful_MessageShouldBeSavedToTheDatabase()
        {
            ...
            var testMessages = new List<MessageQueuePrice>
            { 
                new TestPriceMessageBuilder().WithIrmaPushId(this.testPosData[0].IRMAPushID), 
                new TestPriceMessageBuilder().WithIrmaPushId(this.testPosData[0].IRMAPushID),
                new TestPriceMessageBuilder().WithIrmaPushId(this.testPosData[0].IRMAPushID)
            };

            this.priceMessageQueueService = new PriceMessageQueueService(
                mockLogger.Object,
            ...
            // When.
            this.priceMessageQueueService.SaveMessagesBulk(testMessages);

            // Then.
            var testMessagesById = testMessages.Select(m => m.IRMAPushID).ToList();
            var queuedMessages = this.context.Context.MessageQueuePrice.Where(mq => testMessagesById.Contains(mq.IRMAPushID)).ToList();
        ...
        public void SaveMessagesBulk_TransientErrorOccursAndRetryIsSuccessful_EventsShouldBeLogged()
        {
            ...
            // MessageQueuePrice table before running this test to create the appropriate condition.  The test will be set to auto-pass for automatic runs.
        ...
        public void SaveMessagesBulk_IntransientErrorOccurs_ExceptionShouldBeThrown()
        {
            ...
            this.mockAddMessagesBulkCommandHandler.Setup(c => c.Execute(It.IsAny<AddPriceMessagesBulkCommand>())).Throws(new Exception());
        ...
        public void SaveMessagesRowByRow_SaveIsSuccessfulForEachMessage_MessagesShouldBeSaved()
        {
            ...
            { 
                new TestPriceMessageBuilder().WithIrmaPushId(this.testPosData[0].IRMAPushID), 
                new TestPriceMessageBuilder().WithIrmaPushId(this.testPosData[1].IRMAPushID),
                new TestPriceMessageBuilder().WithIrmaPushId(this.testPosData[2].IRMAPushID)
            };
            ...
            // When.
            this.priceMessageQueueService.SaveMessagesRowByRow(testMessages);
        ...
        public void SaveMessagesRowByRow_SaveFailsForOneMessage_FailedMessageShouldNotBeSaved()
        {
            ...
            // Then.
            var testMessagesById = testMessages.Select(m => m.IRMAPushID).ToList();
            var successfulQueuedMessages = this.context.Context.MessageQueuePrice.Where(mq => testMessagesById.Contains(mq.IRMAPushID)).ToList();
        ...
        public void SaveMessagesRowByRow_SaveFailsForOneMessage_FailedDateShouldBeUpdatedInTheStagingTable()
        {
            ...
            var testMessages = new List<MessageQueuePrice>
            { 
                new TestPriceMessageBuilder().WithIrmaPushId(this.testPosData[0].IRMAPushID).WithMessageTypeId(this.unknownMessageTypeId), 
                new TestPriceMessageBuilder().WithIrmaPushId(this.testPosData[1].IRMAPushID),
                new TestPriceMessageBuilder().WithIrmaPushId(this.testPosData[2].IRMAPushID)
            };

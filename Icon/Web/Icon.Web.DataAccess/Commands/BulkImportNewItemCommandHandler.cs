    ...
    public class BulkImportNewItemCommandHandler : ICommandHandler<BulkImportCommand<BulkImportNewItemModel>>
    {
        ...
        public void Execute(BulkImportCommand<BulkImportNewItemModel> data)
        {
            ...
            var itemListData = data.BulkImportData.ConvertAll(item => new
            {
                ...
                AnimalWelfareRatingId = GetNullableValue(item.AnimalWelfareRatingId),
                Biodynamic = GetNullableValue(item.Biodynamic),
                CheeseMilkTypeId = GetNullableValue(item.CheeseAttributeMilkTypeId),
                CheeseRaw = GetNullableValue(item.CheeseAttributeRaw),
                EcoScaleRatingId = GetNullableValue(item.EcoScaleRatingId),
                GlutenFreeAgencyId = GetNullableValue(item.GlutenFreeAgencyId),
                KosherAgencyId = GetNullableValue(item.KosherAgencyId),
                Msc = GetNullableValue(item.Msc),
                NonGmoAgencyId = GetNullableValue(item.NonGmoAgencyId),
                OrganicAgencyId = GetNullableValue(item.OrganicAgencyId),
                PremiumBodyCare = GetNullableValue(item.PremiumBodyCare),
                SeafoodFreshOrFrozenId = GetNullableValue(item.SeafoodFreshOrFrozenId),
                SeafoodCatchTypeId = GetNullableValue(item.SeafoodWildOrFarmRaisedId),
                VeganAgencyId = GetNullableValue(item.VeganAgencyId),

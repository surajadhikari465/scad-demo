$(function () {
    ...
    let bulkUploadDataType = $('#bulkUploadDataType').val();
    let interval;
    ...
    function setupGrid() {
        $("#bulkUploadStatusGrid").igGrid({
            ...
            fixedHeaders: false,
            columns: [
                { headerText: "ID", key: "BulkUploadId", dataType: "number", width: "3%" },
    ...
    function statusFormatter(val, record) {
        if (val === 'Error') {
            ...
            if (errorRowsCount > 0) {
                return `<a href='/BulkUpload/UploadErrors?bulkUploadDataType=${bulkUploadDataType}&id=${record.BulkUploadId}'>Errors: ${errorRowsCount} </a>`;
            }
            else {
                return `<a href='/BulkUpload/UploadErrors?bulkUploadDataType=${bulkUploadDataType}&id=${record.BulkUploadId}'>Error</a>`;
            }
        }
        else
            return val;
    ...
    function getBulkUploadStatusData(rowCount) {
        $.ajax({
            dataType: "json",
            url: `/BulkUpload/BulkUploadStatus?bulkUploadDataType=${bulkUploadDataType}&rowCount=${rowCount}`,
            data: { rows: rowCount }
        }).done(function (data) {
            ...
            buttons: {
                'Upload':
                {
                    ...
                    click: function () {
                        ...
                        //There is some variable binding issue with bulkUploadDataType so creating an additional variable so that we can pass this to the URL
                        let dataType = $('#bulkUploadDataType').val();
                        var request = $.ajax({
                            xhr: function () {
                            ...
                            url: `/BulkUpload/UploadFiles?bulkUploadDataType=${dataType}`,
                            type: 'POST',

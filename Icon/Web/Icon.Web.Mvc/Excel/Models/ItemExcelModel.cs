    ...
    public class ItemExcelModel : ExcelModel
    {
        ...
        public BulkImportItemModel ConvertToDataModel()
        {
            var item = new BulkImportItemModel
            {
                ...
                Notes = this.Notes,
                AnimalWelfareRatingId = this.GetIdFromDescription(AnimalWelfareRatings.AsDictionary, this.AnimalWelfareRating),
                Biodynamic = this.Biodynamic.GetBoolStringFromCellText(),
                CheeseAttributeMilkTypeId = this.GetIdFromDescription(MilkTypes.AsDictionary, this.CheeseAttributeMilkType),
                CheeseAttributeRaw = this.CheeseAttributeRaw.GetBoolStringFromCellText(),
                EcoScaleRatingId = this.GetIdFromDescription(EcoScaleRatings.AsDictionary, this.EcoScaleRating),
                GlutenFreeAgencyId = this.GetAgencyIdFromText(this.GlutenFree),
                KosherAgencyId = this.GetAgencyIdFromText(this.Kosher),
                Msc = this.Msc.GetBoolStringFromCellText(),
                NonGmoAgencyId = this.GetAgencyIdFromText(this.NonGmo),
                OrganicAgencyId = this.GetAgencyIdFromText(this.Organic),
                PremiumBodyCare = this.PremiumBodyCare.GetBoolStringFromCellText(),
                SeafoodFreshOrFrozenId = this.GetIdFromDescription(SeafoodFreshOrFrozenTypes.AsDictionary, this.SeafoodFreshOrFrozen),
                SeafoodWildOrFarmRaisedId = this.GetIdFromDescription(SeafoodCatchTypes.AsDictionary, this.SeafoodWildOrFarmRaised),
                VeganAgencyId = this.GetAgencyIdFromText(this.Vegan),

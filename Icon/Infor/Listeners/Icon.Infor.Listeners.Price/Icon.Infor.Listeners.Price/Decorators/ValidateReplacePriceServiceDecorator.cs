...
{
    public class ValidateReplacePriceServiceDecorator : IService<PriceModel>
    {
        ...
        public ValidateReplacePriceServiceDecorator(
        ...
        public void Process(IEnumerable<PriceModel> data, IEsbMessage message)
        {
            ...
            if (allReplacePrices.Any())
            {
                ...
                var isValidAddIdDictionary = addPriceIds
                    .ToDictionary(id => id, id => !existingPricesInDb.Select(p => p.GpmID).Contains(id)); // if existing price is not found then it's a valid id
                var isValidDeleteIdDictionary = deletePriceIds
                    .ToDictionary(id => id, id => existingPricesInDb.Select(p => p.GpmID).Contains(id)); // if existing price is found then it's a valid id

                if (isValidAddIdDictionary.Any(d => !d.Value))
                {
                ...
                if (isValidDeleteIdDictionary.Any(d => !d.Value))
                {

...
{
    public class ValidateDeletePriceServiceDecorator : IService<PriceModel>
    {
        ...
        public ValidateDeletePriceServiceDecorator(
        ...
        public void Process(IEnumerable<PriceModel> data, IEsbMessage message)
        {
            ...
            if (prices.Any())
            {
                ...
                List<DbPriceModel> existingPrices = this.getPricesQuery.Search(new GetPricesByGpmIdsParameters { Prices = deletePrices }).ToList();

                if (deletePrices.Count != existingPrices.Count)
                {
                    // Check DeletedPrices table to see if prices were already deleted
                    List<Guid> priceIdsNotFound = prices
                        .Select(dp => dp.GpmId)

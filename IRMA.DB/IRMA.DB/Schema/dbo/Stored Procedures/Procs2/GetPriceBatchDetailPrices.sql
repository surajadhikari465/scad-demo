...
-- 2015-05-20	DN		16164		Reference Item.Retail_Sale instead of PBD.Retail_Sale
--									in the query.
-- 2015-07-05	DN		16250		Concatenation of Ingredient, Allergen & Extra Text fields
-- 2015-07-07	DN		16250		Reorder the fields: Ingredients + Allergen + Extra Text
-- 2015-07-10	DN		16315		Added 2 new columns: ScaleLabelTypeID, Scale_LabelStyle_Desc
...
-- 2015-11-30	KM		VSO 13058	Remove an inner join that was grouping PBD records, causing CancelAllSales records
--									to be lost.
-- 2015-12-03	DN		17729		Added column NewRegPrice. The column will be populated with value when:
--									1. fn_OnSale() > 1 AND
--									2. PriceBatchDetail.Price <> Price.Price 
-- 2016-02-04	KM		13984		Updates for 365 - includes join to ItemCustomerFacingScale and related WHERE condition; also includes
...
CREATE PROCEDURE [dbo].[GetPriceBatchDetailPrices]
	...
	--When this flag is false then the data for all the stores is returned, else only the data for the legacy stores is returned
	--Because the value of this is false, we do not have to add this flag to the ScalePush procedure 
	@LegacyStoresOnly bit = 'False'
AS
	...
	IF @Status = 1 -- Only validated UPCs are passing from Icon to IRMA
		BEGIN
			...
			SELECT 
				...
				Scale_Identifier
			FROM 
				ItemIdentifier II  INNER JOIN ValidatedScanCode VSC  ON II.Identifier = VSC.ScanCode
			...
			SELECT 
				...
				Scale_Identifier
			FROM 
				Item I  INNER JOIN ItemIdentifier II  ON I.Item_Key = II.Item_Key
			WHERE 
				I.Remove_Item = 1 
...
FROM (
		...
		FROM 
			...
			INNER JOIN Item ON D.Item_Key = Item.Item_Key
			INNER JOIN #Stores Store ON Store.Store_No = Price.Store_No
			LEFT JOIN ItemScale ON ItemScale.Item_Key = D.Item_Key
			LEFT JOIN ItemCustomerFacingScale  icfs ON ItemScale.Item_Key = icfs.Item_Key
			LEFT JOIN ItemScaleOverride ISO  ON ISO.Item_Key = Price.Item_Key AND ISO.StoreJurisdictionID = Store.StoreJurisdictionID
					AND @UseRegionalScaleFile = 0 AND @UseStoreJurisdictions = 1
			LEFT JOIN ItemUomOverride IUO  ON D.Item_Key = IUO.Item_Key AND Store.Store_No = IUO.Store_No
			LEFT JOIN Scale_ExtraText ON ISNULL(ISO.Scale_ExtraText_ID, ItemScale.Scale_ExtraText_ID) = Scale_ExtraText.Scale_ExtraText_ID
			LEFT JOIN Scale_LabelType ON Scale_ExtraText.Scale_LabelType_ID = Scale_LabelType.Scale_LabelType_ID
			LEFT JOIN Scale_Ingredient ON ItemScale.Scale_Ingredient_ID = Scale_Ingredient.Scale_Ingredient_ID
			LEFT JOIN Scale_Allergen ON ItemScale.Scale_Allergen_ID = Scale_Allergen.Scale_Allergen_ID
			LEFT JOIN Scale_Tare Scale_Tare ON ISNULL(ISO.Scale_Tare_ID, ItemScale.Scale_Tare_ID) = Scale_Tare.Scale_Tare_ID
			LEFT JOIN Scale_Tare Scale_Tare_Override ON ISO.Scale_Tare_ID = Scale_Tare_Override.Scale_Tare_ID
			LEFT JOIN Scale_Tare Alt_Scale_Tare ON ISNULL(ISO.Scale_Alternate_Tare_ID, ItemScale.Scale_Alternate_Tare_ID) = Alt_Scale_Tare.Scale_Tare_ID
			LEFT JOIN Scale_Grade ON ISNULL(ISO.Scale_Grade_ID, ItemScale.Scale_Grade_ID) = Scale_Grade.Scale_Grade_ID
			LEFT JOIN Scale_LabelStyle ON ISNULL(ISO.Scale_LabelStyle_ID, ItemScale.Scale_LabelStyle_ID) = Scale_LabelStyle.Scale_LabelStyle_ID
			LEFT JOIN ItemUnit ScaleUOM ON ScaleUOM.Unit_ID = COALESCE(IUO.Scale_ScaleUOMUnit_ID, ISO.Scale_ScaleUOMUnit_ID, ItemScale.Scale_ScaleUOMUnit_ID)
			LEFT JOIN LabelType LT ON LT.LabelType_ID = Item.LabelType_ID -- Joins directly on the Item column because a change to this does not trigger an Item PBD record
	...
	LEFT JOIN PricingMethod PM ON PBD.PricingMethod_ID = PM.PricingMethod_ID		
	LEFT JOIN StoreSubTeam SST ON SST.Store_No = PBD.Store_No AND SST.SubTeam_No = PBD.SubTeam_No -- Join on the store exception subteam for POS data
	LEFT JOIN StoreSubTeam SST_Scale ON SST_Scale.Store_No = PBD.Store_No AND SST_Scale.SubTeam_No = DefaultSubteam -- Scale values do not use the exception subteams
	LEFT JOIN SubTeam ST ON SST.SubTeam_No = ST.SubTeam_No
	LEFT JOIN SubTeam ST_Scale ON ST_Scale.SubTeam_No = SST_Scale.SubTeam_No  -- Scale values do not use the exception subteams  
	LEFT JOIN ItemVendor VI ON VI.Vendor_ID = PBD.Vendor_Id AND VI.Item_Key = PBD.Item_Key    
	LEFT JOIN Vendor V ON V.Vendor_ID = PBD.Vendor_Id
	LEFT JOIN dbo.fn_VendorCostAll(@Date) VCA ON VCA.Item_Key = PBD.Item_Key AND VCA.Store_No = PBD.Store_No AND VCA.Vendor_ID = PBD.Vendor_Id
	LEFT JOIN #Stores Store ON Store.Store_No = PBD.Store_No 
	LEFT JOIN Price LP ON LP.Store_No = PBD.Store_No AND LP.Item_Key = PBD.LinkedItem
	LEFT JOIN #ItemIdentifier LII ON PBD.LinkedItem = LII.Item_Key AND LII.Default_Identifier = 1
WHERE 

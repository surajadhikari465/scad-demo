CREATE PROCEDURE dbo.msqBakeryInventoryRaleigh
    ...
    SET Cost = (SELECT SUM(ReceivedItemCost + ReceivedItemFreight) / SUM(UnitsReceived)
                       ...
                       WHERE OrderHeader_ID = 
                             (SELECT TOP 1 OrderHeader.OrderHeader_ID
                              ...
                              WHERE VendStore.Store_No = 10016
                                  AND Return_Order = 0
...
WHERE Vendor.Vendor_ID IN (5527,5319,5255,5563,5596,5485,5617,5464,5273,5868,6052,5669,5671,5281,5713,5967,6035,5969,5058,5793,5971,5732,5737,6240)  
AND SubTeam_No = 4200 AND Price.Store_No = 10016  	      

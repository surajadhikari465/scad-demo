CREATE PROCEDURE dbo.InsertItemHistoryShrinkUpdate 
    @Store_No int, 
    @Item_Key int, 
    @DateStamp datetime, 
...
BEGIN
    ...
    BEGIN
        INSERT INTO ItemHistory (Store_No, Item_Key, DateStamp, Quantity, Weight, ExtCost, Retail, 
                                 Adjustment_ID, AdjustmentReason, CreatedBy, SubTeam_No, InventoryAdjustmentCode_Id, CorrectionRecordFlag)
		...
		ISNULL(dbo.fn_PriceHistoryPrice(Shipper.Item_Key, @Store_No, @DateStamp), 0),
               @Adjustment_ID, @AdjustmentReason, @CreatedBy, @SubTeam_No, @InventoryAdjustmentCode_Id, 1
        FROM Shipper (nolock)
    ...
    BEGIN
        ...
        IF @error_no = 0
        BEGIN
            INSERT INTO ItemHistory (Store_No, Item_Key, DateStamp, Quantity, Weight, ExtCost, Retail, Adjustment_ID, AdjustmentReason, CreatedBy, SubTeam_No, InventoryAdjustmentCode_Id, CorrectionRecordFlag)
            VALUES (@Store_No, @Item_Key, @DateStamp, @Quantity, @Weight, @AvgCost, @Price, @Adjustment_ID, @AdjustmentReason, @CreatedBy, @SubTeam_No, @InventoryAdjustmentCode_Id, 1)
    ...
    BEGIN
        ...
        RAISERROR ('InsertItemHistoryShrinkUpdate failed with @@ERROR: %d', @Severity, 1, @error_no)
    END
END
GO
GRANT EXECUTE
    ON OBJECT::dbo.InsertItemHistoryShrinkUpdate TO IRMAClientRole
    AS dbo;


GO
GRANT EXECUTE
    ON OBJECT::dbo.InsertItemHistoryShrinkUpdate TO IRMASchedJobsRole
    AS dbo;


GO
GRANT EXECUTE
    ON OBJECT::dbo.InsertItemHistoryShrinkUpdate TO IRMAReportsRole
    AS dbo;

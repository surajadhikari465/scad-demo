CREATE PROCEDURE dbo.msqBakeryInventoryWinstonSalem
    ...
    SET Cost = (SELECT SUM(ReceivedItemCost + ReceivedItemFreight) / SUM(UnitsReceived)
                       ...
                       WHERE OrderHeader_ID = 
                             (SELECT TOP 1 OrderHeader.OrderHeader_ID
                              ...
                              WHERE VendStore.Store_No = 10110
                                  AND Return_Order = 0
...
WHERE Vendor.Vendor_ID IN (5527,5319,5255,6080,5596,5868,5318,5801,4997,5971,5058,5732,5793)
AND SubTeam_No = 4200 AND Price.Store_No = 10110	      

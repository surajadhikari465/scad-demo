CREATE PROCEDURE dbo.InsertCycleCountItemsBrand
	@CycleCountID int
	,@SubTeam_No int
	,@Brand_ID int
	,@InvLocID int = null
...
BEGIN TRAN
	...
	BEGIN
		...
		RAISERROR ('InsertCycleCountItemsBrand failed with @@ERROR: %d', @Severity, 1, @Error_No)
	END

SET NOCOUNT OFF
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[InsertCycleCountItemsBrand] TO [IRMAClientRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[InsertCycleCountItemsBrand] TO [IRMASchedJobsRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[InsertCycleCountItemsBrand] TO [IRMAReportsRole]
    AS [dbo];

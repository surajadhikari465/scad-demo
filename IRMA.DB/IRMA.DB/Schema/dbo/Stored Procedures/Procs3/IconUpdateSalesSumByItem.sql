...
SELECT
    ...
    SUM(CASE WHEN a.ItemType IN (0)   
        THEN (a.BasePrice * a.Quantity) + (a.BasePrice * a.Weight)  
        ELSE 0 END) AS Sales_Amount,  
    SUM(CASE WHEN a.ItemType IN (2,3,8)   
        THEN (a.BasePrice * -1 * a.Quantity) + (a.BasePrice * -1 * a.Weight)  
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[IconUpdateSalesSumByItem] TO [IRMAClientRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[IconUpdateSalesSumByItem] TO [IRSUser]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[IconUpdateSalesSumByItem] TO [IConInterface]
    AS [dbo];
